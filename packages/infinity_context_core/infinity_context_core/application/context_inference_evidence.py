...
from infinity_context_core.application.context_inference_evidence_terms import (
    _ANIMAL_CARE_TEXT_TERMS,
    _ANIMAL_CARE_TOPIC_NOISE_TERMS,
    _ANIMAL_CAREER_QUERY_TERMS,
    _BOOK_TOPIC_TEXT_TERMS,
    _CAREER_DECISION_QUERY_TERMS,
    _CAREER_DECISION_TEXT_TERMS,
    _CAREER_DOMAIN_TEXT_TERMS,
    _CAREER_FIELD_TEXT_TERMS,
    _CAREER_INFERENCE_QUERY_TERMS,
    _CAREER_INTENT_TEXT_TERMS,
    _CAREER_NEGATED_DECISION_RE,
    _CAREER_TOPIC_ONLY_TERMS,
    _CAUSAL_EFFECT_TERMS,
    _CAUSAL_QUERY_STOP_TERMS,
    _CAUSAL_QUERY_TERMS,
    _CAUSAL_TEXT_RE,
    _CHILDREN_BOOKS_QUERY_TERMS,
    _CHILDREN_BOOKS_TEXT_TERMS,
    _COUNTERFACTUAL_SUPPORT_DOMAIN_TERMS,
    _COUNTERFACTUAL_SUPPORT_EVIDENCE_TERMS,
    _COUNTERFACTUAL_SUPPORT_QUERY_TERMS,
    _EMOTION_CAUSE_QUERY_TERMS,
    _GENERIC_SUPPORT_ACTION_TERMS,
    _GENERIC_SUPPORT_NOISE_RE,
    _INFERENCE_QUERY_TERMS,
    _INTERPERSONAL_SUPPORT_ACTION_TERMS,
    _MILITARY_SERVICE_TEXT_TERMS,
    _PATRIOTIC_MOTIVE_TEXT_TERMS,
    _PATRIOTIC_QUERY_TERMS,
    _PATRIOTIC_SERVICE_TEXT_TERMS,
    _PUBLIC_OFFICE_SERVICE_TEXT_TERMS,
    _RELIGIOUS_CONTRAST_CONTEXT_TERMS,
    _RELIGIOUS_QUERY_TERMS,
    _RELIGIOUS_STRONG_TEXT_TERMS,
    _RELIGIOUS_TEXT_TERMS,
    _RELIGIOUS_TOPIC_NOISE_TERMS,
    _RELOCATION_WILLINGNESS_QUERY_TERMS,
    _SUPPORT_ROLE_ACTION_TERMS,
    _SUPPORT_ROLE_OPERATIONAL_NOISE_RE,
    _SUPPORT_ROLE_SCENARIO_TERMS,
    _TOKEN_RE,
    _WILLINGNESS_MARKER_TERMS,
    _WILLINGNESS_QUERY_TERMS,
    _WILLINGNESS_TEXT_DOMAIN_TERMS,
)
...
def _causal_answer_signal(*, query: str, text: str) -> AnswerEvidenceSignal:
    ...
    conflict_signal = preference_polarity_conflict_signal(query=query, text=text)
    if conflict_signal.reason:
        return conflict_signal
    salient_terms = _causal_salient_terms(query_tokens)
    salient_overlap = bool(salient_terms & text_tokens)
    marker_salient_overlap = _causal_marker_has_salient_overlap(
        text=text,
    ...
    if has_causal_marker and salient_overlap and not marker_salient_overlap:
        return AnswerEvidenceSignal(
            penalty=0.028,
            reason="causal_answer_unrelated_reason_signal",
        )
    if has_causal_marker and (marker_salient_overlap or emotion_cause_match):
        return AnswerEvidenceSignal(
            boost=0.03,
...
def _causal_marker_has_salient_overlap(
    *,
    text: str,
    ...
    for sentence in re.split(r"(?<=[.!?])\s+|\n+", text):
        if not _CAUSAL_TEXT_RE.search(sentence):
            continue
        if _term_set(sentence) & salient_terms:
            return True
    return False
