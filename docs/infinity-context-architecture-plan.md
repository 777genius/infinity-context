...
| Locator retrieval | Retrieval and Context Packing, Canonical Visibility and Read-Path Guard Model, ADR-0011 | only opaque locators and bounded retrieval provenance cross Retrieval V2; canonical hydration remains lifecycle authority |
...
| Retrieval V2 is locator-only | lets consumers reuse generic query execution and fusion without exporting prompt evidence or downstream authority | `context_building` Retrieval V2 domain/application/ports and versioned contracts | forbidden response keys, deterministic weighted RRF, canonical lifecycle/filter hydration, neighbor and cancellation tests | text, stale candidates or authorization-like claims cross the service boundary |
| Retrieval V2 admission and recovery are canonical | prevents transition races and crashed runtime owners from bypassing or permanently blocking the active profile | PostgreSQL maintenance/evidence gates, exact runtime owner generations, create-only recovery receipts | activation-first query, ambiguous mutation, stale-owner recovery and populated 0045 upgrade tests | unfenced legacy routing, provider/delete races or permanent lifecycle outage |
