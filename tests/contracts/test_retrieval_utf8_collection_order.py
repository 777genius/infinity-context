...
def test_raw_json_recursively_decodes_surrogate_pairs_as_unicode_scalars() -> None:
    decoded = decode_context_retrieval_json(
        b'{"\\ud83d\\ude00":{"nested":["\\ud83d\\ude00",{"value":"e\\u0301"}]}}'
    )
...
def test_raw_json_rejects_unpaired_or_reversed_surrogates_with_value_error(
    ...
    with pytest.raises(ValueError, match="unpaired Unicode surrogate"):
        decode_context_retrieval_json(raw)
...
def test_valid_supplementary_root_string_reaches_object_shape_guard() -> None:
    with pytest.raises(ValueError, match="must contain one object"):
        decode_context_retrieval_json(b'"\\ud83d\\ude00"')
    with pytest.raises(ValueError, match="unpaired Unicode surrogate"):
        decode_context_retrieval_json(b'"\\ud800"')
...
def test_capability_fingerprint_scalar_decoding_and_duplicate_detection() -> None:
    escaped_form = {"key": "\ud83d\ude00"}
    scalar_form = {"key": "😀"}
    assert capability_fingerprint(escaped_form) == capability_fingerprint(scalar_form)
    with pytest.raises(ValueError, match="duplicate key: 😀"):
        capability_fingerprint({"\ud83d\ude00": 1, "😀": 2})
    with pytest.raises(ValueError, match="unpaired Unicode surrogate"):
        capability_fingerprint({"nested": ["\ud800"]})
...
"""
    python_values = [decode_context_retrieval_json(item)["key"] for item in (escaped, raw)]
    node_values = [
        subprocess.run(
...
def test_every_dto_preference_key_array_requires_utf8_canonical_input(
    ...
    weighted = tuple(RetrievalWeightedKeyDto(value) for value in values)
    accepted = RetrievalSoftPreferencesDto(**{field: weighted})
    assert tuple(item.key for item in getattr(accepted, field)) == values
    with pytest.raises(ValueError, match="UTF-8 sorted unique"):
        RetrievalSoftPreferencesDto(**{field: tuple(reversed(weighted))})
    with pytest.raises(ValueError, match="UTF-8 sorted unique"):
        RetrievalSoftPreferencesDto(**{field: (weighted[0], weighted[0])})
...
def test_every_core_preference_key_array_requires_utf8_canonical_input(
    ...
    weighted = tuple(core.LocatorWeightedKey(value) for value in values)
    accepted = core.LocatorSoftPreferences(**{field: weighted})
    assert tuple(item.key for item in getattr(accepted, field)) == values
    with pytest.raises(ValueError, match="UTF-8 sorted unique"):
        core.LocatorSoftPreferences(**{field: tuple(reversed(weighted))})
    with pytest.raises(ValueError, match="UTF-8 sorted unique"):
        core.LocatorSoftPreferences(**{field: (weighted[0], weighted[0])})


@pytest.mark.parametrize("values", CANONICAL_PROBES)
def test_dto_and_core_source_generation_pair_keys_use_utf8_order(values: tuple[str, str]) -> None:
    dto_pairs = tuple(RetrievalSourceGenerationDto(value, f"g-{value}") for value in values)
    core_pairs = tuple(core.LocatorSourceGeneration(value, f"g-{value}") for value in values)
    assert RetrievalHardFiltersDto(dto_pairs).source_generations == dto_pairs
    assert core.LocatorHardFilters(core_pairs).source_generations == core_pairs
    with pytest.raises(ValueError, match="sorted"):
        RetrievalHardFiltersDto(tuple(reversed(dto_pairs)))
    with pytest.raises(ValueError, match="sorted"):
        core.LocatorHardFilters(tuple(reversed(core_pairs)))

    dto_same_source = tuple(RetrievalSourceGenerationDto("source", value) for value in values)
    core_same_source = tuple(core.LocatorSourceGeneration("source", value) for value in values)
    with pytest.raises(ValueError, match="sorted"):
        RetrievalHardFiltersDto(tuple(reversed(dto_same_source)))
    with pytest.raises(ValueError, match="sorted"):
        core.LocatorHardFilters(tuple(reversed(core_same_source)))
    with pytest.raises(ValueError, match="unique source keys"):
        RetrievalHardFiltersDto((dto_same_source[0], dto_same_source[0]))
    with pytest.raises(ValueError, match="unique source keys"):
        core.LocatorHardFilters((core_same_source[0], core_same_source[0]))
...
def test_request_query_ids_are_utf8_sorted_distinct_and_not_normalized(
    ...
    dto_queries = tuple(RetrievalQueryDto(value, f"query {value}") for value in values)
    core_queries = tuple(core.LocatorQueryVariant(value, f"query {value}") for value in values)
...
def test_response_keyed_collections_fail_closed_without_changing_rank_order() -> None:
    ...
    outcomes = (
        RetrievalProviderOutcomeDto("\ue000", "unavailable", "provider_unavailable"),
        RetrievalProviderOutcomeDto("\U00010000", "unavailable", "provider_unavailable"),
    )
...
def test_core_response_keyed_collections_match_dto_guards() -> None:
    ...
    outcomes = (
        core.LocatorProviderOutcome("\ue000", "unavailable", "provider_unavailable"),
        core.LocatorProviderOutcome("\U00010000", "unavailable", "provider_unavailable"),
    )
...
def test_capability_provider_ids_are_utf8_sorted_in_dto_and_core(values: tuple[str, str]) -> None:
    ...
    assert (
        tuple(lane.provider_id for lane in RetrievalCapabilityDto.from_dict(payload).provider_lanes)
        == values
    )

    lanes = tuple(
        core.LocatorProviderLaneCapability(value, True, True, 1_000_000, True) for value in values
    )
    ...
    for invalid in (tuple(reversed(values)), (values[0], values[0])):
        ...
        invalid_payload["capability_fingerprint"] = capability_fingerprint(invalid_payload)
        with pytest.raises(ValueError, match="provider_lanes must be sorted and unique"):
            RetrievalCapabilityDto.from_dict(invalid_payload)
        invalid_lanes = tuple(
            core.LocatorProviderLaneCapability(value, True, True, 1_000_000, True)
            for value in invalid
        )
        with pytest.raises(ValueError, match="provider lanes must be sorted and unique"):
            core.LocatorRetrievalCapability(
                FINGERPRINT,
                "profile",
                service_revision="b" * 40,
                index_profile_digest="c" * 64,
...
def _dto_filters(**changes: tuple[str, ...]) -> RetrievalHardFiltersDto:
    return RetrievalHardFiltersDto(
        (RetrievalSourceGenerationDto("source", "generation"),), **changes
    )


def _core_filters(**changes: tuple[str, ...]) -> core.LocatorHardFilters:
    return core.LocatorHardFilters(
        (core.LocatorSourceGeneration("source", "generation"),), **changes
    )


def _dto_request(queries: tuple[RetrievalQueryDto, ...]) -> RetrieveContextRequestDto:
    return RetrieveContextRequestDto(
        LOCATOR_RETRIEVAL_CONTRACT_VERSION,
        FINGERPRINT,
        "profile",
...
def _core_request(
    queries: tuple[core.LocatorQueryVariant, ...],
...
def _canonical_candidate(**changes: tuple[str, ...]) -> core.CanonicalLocatorCandidate:
    values: dict[str, object] = {
        "locator": "locator",
    ...
    values.update(changes)
    return core.CanonicalLocatorCandidate(**values)  # type: ignore[arg-type]


def _dto_contribution(provider: str, query: str, score: float) -> RetrievalContributionDto:
...
def _dto_candidate(
    contributions: tuple[RetrievalContributionDto, ...], queries: tuple[str, ...]
) -> RetrievalCandidateDto:
...
def _dto_empty_response(
    *,
    outcomes: tuple[RetrievalProviderOutcomeDto, ...] = (),
    reasons: tuple[str, ...] = (),
) -> RetrieveContextResponseDto:
...
def _core_contribution(provider: str, query: str, score: float) -> core.LocatorScoreContribution:
    return core.LocatorScoreContribution(
        provider, query, 1, 1_000_000, 1_000_000, round(score * 1_000_000_000_000)
    )


def _core_result_candidate(
    contributions: tuple[core.LocatorScoreContribution, ...], queries: tuple[str, ...]
) -> core.LocatorResultCandidate:
    return core.LocatorResultCandidate(
        "locator",
        "source",
        "document",
        "chunk",
...
def _core_empty_response(
    *,
    outcomes: tuple[core.LocatorProviderOutcome, ...] = (),
    reasons: tuple[str, ...] = (),
    ...
    return core.LocatorRetrievalResponse(
        ...
        FINGERPRINT,
        "profile",
        core.LocatorAppliedBounds(1, 1, 0, 16_384, 1, 0, 0),
        (),
        outcomes,
        reasons,
    )
