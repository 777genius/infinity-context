...
def _suite(config, projection, extraction, readiness) -> SchedulerSuiteAuthority:
    ...
    if tuple(item.dataset_sha256 for item in terminals) != expected_dataset_sha:
    ...
    bindings = []
    for index, (scheduler_profile, run_id, terminal) in enumerate(
        zip(scheduler_profiles, run_ids, terminals, strict=True)
    ):
        ...
        if terminal.run_id_sha256 != hashlib.sha256(run_id.encode()).hexdigest():
        ...
        bindings.append(
            SchedulerRunBinding(
                ...
                dataset_sha256=terminal.dataset_sha256,
