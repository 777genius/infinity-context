"""Internal implementation of versioned locator-only Retrieval DTOs."""
...
from ._context_building_retrieval_filters import (
...
MIN_LOCATOR_RETRIEVAL_RESPONSE_BYTES = 16_384
...
class RetrievalBoundsDto:
    ...
    def __post_init__(self) -> None:
        ...
        if not MIN_LOCATOR_RETRIEVAL_RESPONSE_BYTES <= self.response_byte_limit <= 1_048_576:
...
class RetrieveContextRequestDto:
    ...
    queries: Sequence[RetrievalQueryDto]
    filters: RetrievalHardFiltersDto
    soft_preferences: RetrievalSoftPreferencesDto = field(
        default_factory=RetrievalSoftPreferencesDto
    )
    bounds: RetrievalBoundsDto = field(default_factory=RetrievalBoundsDto)
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.scope, RetrievalScopeDto):
        ...
        raw_queries = _canonical_tuple(self.queries, RetrievalQueryDto, "queries")
        queries = tuple(
            RetrievalQueryDto(item.query_id, item.query, item.weight_micros) for item in raw_queries
        )
        if not isinstance(self.filters, RetrievalHardFiltersDto):
        ...
        if not isinstance(self.soft_preferences, RetrievalSoftPreferencesDto):
            raise ValueError("soft_preferences has an invalid runtime type")
        soft_preferences = RetrievalSoftPreferencesDto(
        ...
        if not isinstance(self.bounds, RetrievalBoundsDto):
    ...
    @classmethod
    def from_dict(cls, payload: Mapping[str, object]) -> RetrieveContextRequestDto:
        ...
        if contract_version != LOCATOR_RETRIEVAL_CONTRACT_VERSION:
            raise ValueError("contract_version is unsupported")
...
@dataclass(frozen=True, slots=True)
class RetrievalContributionDto:
    provider_id: str
    query_id: str
    provider_rank: int
...
class RetrievalCandidateDto:
    ...
    contributions: Sequence[RetrievalContributionDto] = field(default_factory=tuple)
    base_score_picos: int | None = None
    neighbors: Sequence[RetrievalNeighborDto] = field(default_factory=tuple)
    ...
    def __post_init__(self) -> None:
        ...
        raw_contributions = _canonical_tuple(
            self.contributions, RetrievalContributionDto, "candidate.contributions"
        )
        contributions = tuple(
        ...
        raw_neighbors = _canonical_tuple(
            self.neighbors, RetrievalNeighborDto, "candidate.neighbors"
        )
        neighbors = tuple(
...
@dataclass(frozen=True, slots=True)
class RetrievalProviderOutcomeDto:
    provider_id: str
    status: str
...
@dataclass(frozen=True, slots=True)
class RetrievalAppliedBoundsDto:
    candidate_limit: int
    result_limit: int
    neighbor_radius: int
...
@dataclass(frozen=True, slots=True)
class RetrieveContextResponseDto:
    status: str
    capability_fingerprint: str
    profile_id: str
    applied_bounds: RetrievalAppliedBoundsDto
    candidates: Sequence[RetrievalCandidateDto]
    provider_outcomes: Sequence[RetrievalProviderOutcomeDto]
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.applied_bounds, RetrievalAppliedBoundsDto):
        ...
        raw_candidates = _canonical_tuple(self.candidates, RetrievalCandidateDto, "candidates")
        candidates = tuple(
        ...
        raw_outcomes = _canonical_tuple(
            self.provider_outcomes, RetrievalProviderOutcomeDto, "provider_outcomes"
        )
        outcomes = tuple(
    ...
    @classmethod
    def from_dict(cls, payload: Mapping[str, object]) -> RetrieveContextResponseDto:
        from ._context_building_retrieval_response import (  # noqa: PLC0415
            parse_retrieve_context_response,
        )

        return parse_retrieve_context_response(payload)


def _parse_query(payload: Mapping[str, object], index: int) -> RetrievalQueryDto:
...
def _parse_bounds(payload: Mapping[str, object]) -> RetrievalBoundsDto:
    _require_exact(
        payload,
        {
            "candidate_limit",
    ...
    if not 0 <= neighbor_radius <= 2:
        raise ValueError("bounds.neighbor_radius must be within 0..2")
    if not MIN_LOCATOR_RETRIEVAL_RESPONSE_BYTES <= response_bytes <= 1_048_576:
        raise ValueError("bounds.response_byte_limit must be within 16384..1048576")
    if not 1 <= deadline_ms <= 2_000:
        raise ValueError("bounds.deadline_ms must be within 1..2000")
    return RetrievalBoundsDto(
        candidate_limit, result_limit, neighbor_radius, response_bytes, deadline_ms
    )


def _copy_time_interval(
    value: RetrievalTimeIntervalDto | None, path: str
) -> RetrievalTimeIntervalDto | None:
    if value is None:
        return None
    if not isinstance(value, RetrievalTimeIntervalDto):
        raise ValueError(f"{path} has an invalid runtime type")
    return RetrievalTimeIntervalDto(value.start_at, value.end_at)


def _copy_relative_time_interval(
    value: RetrievalRelativeTimeIntervalDto | None, path: str
) -> RetrievalRelativeTimeIntervalDto | None:
    if value is None:
        return None
    if not isinstance(value, RetrievalRelativeTimeIntervalDto):
        raise ValueError(f"{path} has an invalid runtime type")
    return RetrievalRelativeTimeIntervalDto(value.start_ms, value.end_ms)
...
__all__ = [
    ...
    "MIN_LOCATOR_RETRIEVAL_RESPONSE_BYTES",
    "RetrievalAppliedBoundsDto",
    "RetrievalBoundsDto",
    "RetrievalCandidateDto",
    "RetrievalContributionDto",
