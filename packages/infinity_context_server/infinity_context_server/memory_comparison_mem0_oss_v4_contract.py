"""Strict provider-free validation for the additive Mem0 OSS v4 runtime."""
...
MEM0_OSS_V4_RUNTIME_CAPABILITY_ISSUE_CODES = frozenset(
    {
        "oss_v4_manifest_not_object",
        "oss_v4_unknown_top_level_fields",
        "oss_v4_missing_top_level_fields",
        "oss_v4_schema_version_mismatch",
        "oss_v4_configured_not_true",
        "oss_v4_wrapper_source_revision_invalid",
        "oss_v4_wrapper_source_revision_mismatch",
        "oss_v4_wrapper_source_sha256_invalid",
        "oss_v4_wrapper_source_sha256_mismatch",
        "oss_v4_config_fingerprint_sha256_invalid",
        "oss_v4_adapter_mismatch",
        "oss_v4_runtime_invalid",
        "oss_v4_runtime_mode_mismatch",
        "oss_v4_runtime_auth_mode_invalid",
        "oss_v4_runtime_python_version_mismatch",
        "oss_v4_runtime_python_image_digest_mismatch",
        "oss_v4_runtime_qdrant_image_digest_mismatch",
        "oss_v4_packages_invalid",
        "oss_v4_mem0ai_package_mismatch",
        "oss_v4_fastembed_package_mismatch",
        "oss_v4_qdrant_client_package_mismatch",
        "oss_v4_embedding_invalid",
        "oss_v4_embedding_mismatch",
        "oss_v4_extraction_invalid",
        "oss_v4_extraction_mode_invalid",
        "oss_v4_extraction_cap_invalid",
        "oss_v4_extraction_subscription_scope_invalid",
        "oss_v4_extraction_telemetry_enabled",
        "oss_v4_usage_ledger_invalid",
        "oss_v4_usage_evidence_invalid",
        "oss_v4_timestamp_invalid",
        "oss_v4_timestamp_mode_invalid",
        "oss_v4_timestamp_sdk_native_invalid",
        "oss_v4_timestamp_request_not_supported",
        "oss_v4_timestamp_readback_not_supported",
        "oss_v4_timestamp_unbound_attestation_invalid",
        "oss_v4_timestamp_refresh_attestation_invalid",
        "oss_v4_persisted_source_identity_invalid",
        "oss_v4_persisted_source_identity_static_invalid",
        "oss_v4_persisted_source_identity_unbound_invalid",
        "oss_v4_persisted_source_identity_refresh_invalid",
        "oss_v4_capabilities_invalid",
        "oss_v4_delete_invalid",
        "oss_v4_integrity_invalid",
        "oss_v4_runtime_pin_sha256_mismatch",
        "oss_v4_lock_sha256_mismatch",
        "oss_v4_manifest_sha256_mismatch",
        "oss_v4_refresh_witness_without_binding",
        "oss_v4_refresh_binding_invalid",
        "oss_v4_refresh_witness_invalid",
        "oss_v4_refresh_witness_fingerprint_mismatch",
        "oss_v4_refresh_timestamp_binding_mismatch",
    }
)


def evaluate_mem0_oss_v4_runtime_capabilities(
    payload: object,
    *,
    require_timestamp: bool,
) -> tuple[str, ...]:
    """Validate one exact v4 capability manifest without provider imports.

    `require_timestamp` remains part of the common contract signature. v4,
    like v3, requires wrapper-managed timestamp evidence and allows the static
    pre-refresh profile only as an unbound manifest.
    """

    del require_timestamp
    if not isinstance(payload, Mapping):
        return ("oss_v4_manifest_not_object",)
    issues: list[str] = []
...
def _validate_top_level(payload: Mapping[str, object], issues: list[str]) -> None:
    keys = set(payload)
    if keys.difference(MEM0_OSS_V4_BASE_FIELDS | MEM0_OSS_V4_REFRESH_FIELDS):
        issues.append("oss_v4_unknown_top_level_fields")
    if MEM0_OSS_V4_BASE_FIELDS.difference(keys):
        issues.append("oss_v4_missing_top_level_fields")
    if "refresh_witness" in keys and "refresh_binding" not in keys:
        issues.append("oss_v4_refresh_witness_without_binding")
...
def _validate_scalar_provenance(payload: Mapping[str, object], issues: list[str]) -> None:
    if payload.get("schema_version") != MEM0_BENCHMARK_CAPABILITIES_SCHEMA_VERSION_V4:
        issues.append("oss_v4_schema_version_mismatch")
    if payload.get("runtime_mode") != MEM0_OSS_RUNTIME_MODE:
        issues.append("oss_v4_runtime_mode_mismatch")
    if payload.get("configured") is not True:
        issues.append("oss_v4_configured_not_true")
    revision = payload.get("wrapper_source_revision")
    if not is_revision(revision):
        issues.append("oss_v4_wrapper_source_revision_invalid")
    elif revision != REVIEWED_MEM0_OSS_V4_WRAPPER_SOURCE_REVISION:
        issues.append("oss_v4_wrapper_source_revision_mismatch")
    wrapper_sha256 = payload.get("wrapper_source_sha256")
    if not is_sha256(wrapper_sha256):
        issues.append("oss_v4_wrapper_source_sha256_invalid")
    elif wrapper_sha256 != REVIEWED_MEM0_OSS_V4_WRAPPER_SOURCE_SHA256:
        issues.append("oss_v4_wrapper_source_sha256_mismatch")
    if not is_sha256(payload.get("config_fingerprint_sha256")):
        issues.append("oss_v4_config_fingerprint_sha256_invalid")
    if payload.get("adapter") != MEM0_OSS_ADAPTER:
        issues.append("oss_v4_adapter_mismatch")
...
def _validate_extraction(value: object, issues: list[str]) -> None:
    ...
    if not _valid_usage_ledger(extraction.get("usage_ledger")):
        issues.append("oss_v4_usage_ledger_invalid")
    if not exact_mapping_matches(extraction.get("usage_evidence"), MEM0_OSS_V4_USAGE_EVIDENCE):
        issues.append("oss_v4_usage_evidence_invalid")
...
def _validate_timestamp(
    ...
    *,
    ...
    if timestamp is None:
        issues.append("oss_v4_timestamp_invalid")
        return
    if timestamp.get("mode") != "wrapper_metadata_created_at":
        issues.append("oss_v4_timestamp_mode_invalid")
    if timestamp.get("sdk_native_timestamp") is not False:
        issues.append("oss_v4_timestamp_sdk_native_invalid")
    if timestamp.get("request_supported") is not True:
        issues.append("oss_v4_timestamp_request_not_supported")
    if timestamp.get("metadata_readback_supported") is not True:
        issues.append("oss_v4_timestamp_readback_not_supported")
    valid = (
    ...
    if not valid:
        issues.append(
            "oss_v4_timestamp_refresh_attestation_invalid"
            if refresh_bound
            else "oss_v4_timestamp_unbound_attestation_invalid"
        )
...
def _validate_persisted_source_identity(
    ...
    *,
    ...
    if identity is None:
        issues.append("oss_v4_persisted_source_identity_invalid")
        return
    if (
        ...
        issues.append("oss_v4_persisted_source_identity_static_invalid")
    ...
    if (
        ...
        issues.append(
            "oss_v4_persisted_source_identity_refresh_invalid"
            if refresh_bound
            else "oss_v4_persisted_source_identity_unbound_invalid"
        )


def _validate_capabilities(value: object, issues: list[str]) -> None:
    if not isinstance(value, list) or tuple(value) != MEM0_OSS_V4_CAPABILITIES:
        issues.append("oss_v4_capabilities_invalid")
...
def _validate_delete(value: object, issues: list[str]) -> None:
    ...
    if delete is None or delete.get("verified_absent") is not True:
        issues.append("oss_v4_delete_invalid")
...
def _validate_integrity(payload: Mapping[str, object], issues: list[str]) -> None:
    ...
    if integrity is None or not all(is_sha256(integrity.get(key)) for key in integrity):
        issues.append("oss_v4_integrity_invalid")
        return
    if integrity.get("runtime_pin_sha256") != REVIEWED_MEM0_OSS_V4_RUNTIME_PIN_SHA256:
        issues.append("oss_v4_runtime_pin_sha256_mismatch")
    if integrity.get("lock_sha256") != REVIEWED_MEM0_OSS_V4_LOCK_SHA256:
        issues.append("oss_v4_lock_sha256_mismatch")
    if integrity.get("manifest_sha256") != mem0_oss_v4_runtime_manifest_sha256(payload):
        issues.append("oss_v4_manifest_sha256_mismatch")
...
def _validate_refresh_evidence(payload: Mapping[str, object], issues: list[str]) -> None:
    ...
    if (
        ...
        issues.append("oss_v4_refresh_witness_invalid")
        return
    if witness.get("manifest_fingerprint_sha256") != mem0_oss_v4_refresh_manifest_sha256(payload):
        issues.append("oss_v4_refresh_witness_fingerprint_mismatch")
...
def _valid_extraction_mode(extraction: Mapping[str, object], mode: str) -> bool:
    ...
    return (
        ...
        and isinstance(extraction.get("max_calls_per_add"), int)
        and not isinstance(extraction.get("max_calls_per_add"), bool)
...
def _valid_non_negative_bound(value: object) -> bool:
    return (
        ...
        and 0 < value <= MEM0_OSS_MAX_LEDGER_BYTES
    )
