"""Feature-owned filter and time-coordinate policy for locator Retrieval."""
...
MIN_PREFERENCE_WEIGHT_MICROS = 100_000
MAX_PREFERENCE_WEIGHT_MICROS = 10_000_000
PREFERENCE_SCORE_SCALE = 1_000_000
MAX_PREFERENCE_BOOST_MICROS = 250_000


@dataclass(frozen=True, slots=True)
class LocatorTimeInterval:
    start_at: datetime
    end_at: datetime
...
@dataclass(frozen=True, slots=True)
class LocatorRelativeTimeInterval:
    start_ms: int
    end_ms: int
...
@dataclass(frozen=True, slots=True)
class LocatorPreferenceEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        if not 0 <= self.score_micros <= PREFERENCE_SCORE_SCALE:
            raise ValueError("Locator preference score_micros is out of bounds")
        expected = self.score_micros * MAX_PREFERENCE_BOOST_MICROS // PREFERENCE_SCORE_SCALE
        ...
        expected_score = 0 if requested == 0 else matched * PREFERENCE_SCORE_SCALE // requested
        if self.score_micros != expected_score:
            raise ValueError("Locator preference score_micros must reconstruct")


@dataclass(frozen=True, slots=True)
class LocatorHardFilters:
    source_generations: tuple[LocatorSourceGeneration, ...]
    ...
    time_interval: LocatorTimeInterval | None = None
    relative_time_interval: LocatorRelativeTimeInterval | None = None

    def __post_init__(self) -> None:
        raw_pairs = _tuple(self.source_generations, LocatorSourceGeneration, "source_generations")
        pairs = tuple(
...
@dataclass(frozen=True, slots=True)
class LocatorSoftPreferences:
    source_preferences: tuple[LocatorWeightedKey, ...] = ()
    actor_preferences: tuple[LocatorWeightedKey, ...] = ()
    time_interval: LocatorTimeInterval | None = None
    relative_time_interval: LocatorRelativeTimeInterval | None = None
    ...
    def __post_init__(self) -> None:
        for name in ("source_preferences", "actor_preferences"):
            raw = _tuple(getattr(self, name), LocatorWeightedKey, name)
            values = tuple(LocatorWeightedKey(item.key, item.weight_micros) for item in raw)
...
def preference_evidence(
    ...
    *,
    ...
    score_micros = 0 if total == 0 else matched * PREFERENCE_SCORE_SCALE // total
    return LocatorPreferenceEvidence(
        score_micros,
        score_micros * MAX_PREFERENCE_BOOST_MICROS // PREFERENCE_SCORE_SCALE,
        source_total,
        source_matched,
        actor_total,
        actor_matched,
...
def _interval_types(
    absolute: LocatorTimeInterval | None,
    relative: LocatorRelativeTimeInterval | None,
    path: str,
) -> None:
    if absolute is not None and not isinstance(absolute, LocatorTimeInterval):
        raise ValueError(f"Locator {path} absolute interval has an invalid type")
    if relative is not None and not isinstance(relative, LocatorRelativeTimeInterval):
...
def _weight_micros(name: str, value: object) -> None:
    ...
    if not MIN_PREFERENCE_WEIGHT_MICROS <= value <= MAX_PREFERENCE_WEIGHT_MICROS:
...
__all__ = (
    ...
    "MAX_PREFERENCE_WEIGHT_MICROS",
    "MIN_PREFERENCE_WEIGHT_MICROS",
