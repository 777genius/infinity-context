...
import {
  retrievalContributionScorePicos,
  retrievalPreferenceScores,
  roundHalfEvenDivide,
} from "../src/retrieval-numeric.js";

const fixtureUrl = new URL("../fixtures/context_retrieval_v2/scoring_golden.json", import.meta.url);

describe("Retrieval canonical integer scoring", () => {
  ...
  it("matches every shared weighted-RRF contribution oracle", async () => {
    ...
    for (const item of fixture.contribution_cases) {
      expect(retrievalContributionScorePicos(
  ...
  it("matches every shared preference and rerank oracle", async () => {
    ...
    for (const item of fixture.composite_cases) {
      ...
      expect(retrievalPreferenceScores(base, requested, matched), item.case_id).toEqual({
  ...
  it("uses BigInt before narrowing products to safe wire integers", () => {
    expect(retrievalContributionScorePicos(10_000_000, 10_000_000, 10_000_000, 1))
      .toBe(163_934_426_230);
    expect(() => retrievalPreferenceScores(Number.MAX_SAFE_INTEGER, 1, 1)).toThrow();
  });
