...
if (JSON.stringify(fixtureNames) !== JSON.stringify(expectedNames)) {
  throw new Error(`Retrieval V2 fixture set drifted: ${fixtureNames.join(", ")}`);
}
for (const name of expectedNames) {
  const bytes = await readFile(new URL(name, fixtureRoot));
  const actual = createHash("sha256").update(bytes).digest("hex");
  if (actual !== expectedFixtureSha256[name]) {
    throw new Error(`Retrieval V2 fixture bytes drifted: ${name} (${actual})`);
  }
}
...
if (!/method:\s*"POST"[\s\S]{0,300}?path:\s*"\/v1\/context\/retrieve"/u.test(contextResource)) {
  throw new Error("Retrieval V2 SDK endpoint parity failed: POST /v1/context/retrieve is missing");
}
const pythonClient = await readFile(
  new URL("../../infinity_context_sdk/infinity_context_sdk/retrieval_v2.py", import.meta.url),
  "utf8",
);
if (!pythonClient.includes("def retrieve_context_v2(") ||
  ...
  throw new Error("Retrieval V2 Python SDK endpoint parity failed");
}
if (!pythonClient.includes("decode_retrieve_context_v2_response") ||
    !pythonClient.includes("RetrievalV2CapabilityDto.from_dict")) {
  throw new Error("Retrieval V2 Python SDK strict response/capability parity failed");
}
...
if (packageJson.exports?.["./fixtures/context_retrieval_v2/*.json"]?.default !==
  ...
  throw new Error("Retrieval V2 fixture package export is missing");
}

console.log(`Retrieval V2 parity ok: Python/TypeScript POST /v1/context/retrieve and ${expectedNames.length} canonical fixtures`);
