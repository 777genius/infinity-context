...
import { retrievalV2ContributionScorePicos, retrievalV2PreferenceScores } from "./retrieval-v2-numeric.js";
import { requestedPreferenceEvidence, validatePreferenceEvidence } from "./retrieval-v2-preferences.js";
import { decodeContextRetrievalV2Json } from "./retrieval-v2-json.js";
import {
...
} from "./retrieval-v2-validation.js";
import type { JsonObject, JsonValue } from "./types.js";
import {
  CONTEXT_RETRIEVAL_CONTRACT_V2,
  CONTEXT_RETRIEVAL_RANKING_POLICY_V2,
  type ContextRetrievalCapabilityV2,
  type RequiredContextRetrievalCapabilityV2,
  type RetrievalV2AppliedBounds,
  type RetrievalV2BoundsInput,
  type RetrievalV2Candidate,
  type RetrievalV2CapabilityBounds,
  type RetrievalV2CapabilityProviderLane,
  type RetrievalV2Contribution,
  type RetrievalV2DegradationReasonCode,
  type RetrievalV2HardFiltersInput,
  type RetrievalV2HardFilterSignal,
  type RetrievalV2Neighbor,
  type RetrievalV2ProviderOutcome,
  type RetrievalV2ProviderReasonCode,
  type RetrievalV2ProviderStatus,
  type RetrievalV2QueryInput,
  type RetrievalV2RankingParameters,
  type RetrievalV2RawScoreKind,
  type RetrievalV2ScopeInput,
  type RetrievalV2SoftPreferencesInput,
  type RetrievalV2SoftPreferenceSignal,
  type RetrievalV2TimeIntervalInput,
  type RetrievalV2RelativeTimeIntervalInput,
  type RetrievalV2SourceGenerationInput,
  type RetrievalV2WeightedKeyInput,
...
] as const satisfies readonly RetrievalV2HardFilterSignal[];
const SOFT_SIGNALS = ["actor_preferences", "relative_time_interval", "source_preferences", "time_interval"] as const satisfies readonly RetrievalV2SoftPreferenceSignal[];
...
] as const satisfies readonly RetrievalV2DegradationReasonCode[];
const RAW_SCORE_KINDS = ["similarity", "distance", "relevance", "bm25"] as const satisfies readonly RetrievalV2RawScoreKind[];
...
} as const satisfies RetrievalV2RankingParameters);

export function retrievalV2RequestPayload(input: RetrieveContextV2Input): JsonObject {
  const root = exactObject(input, [
    "contractVersion", "capabilityFingerprint", "profileId", "scope", "queries", "filters",
    "softPreferences", "bounds",
  ], "input");
  literal(root.contractVersion, CONTEXT_RETRIEVAL_CONTRACT_V2, "input.contractVersion");
...
export function decodeRetrieveContextV2Response(
  payload: unknown,
  request: RetrieveContextV2Input,
  capability: ContextRetrievalCapabilityV2,
  receivedResponseBytes: number,
): RetrieveContextV2Response {
  const requestPayload = retrievalV2RequestPayload(request);
  return decodeRetrieveContextV2ResponseForPayload(payload, requestPayload, capability, receivedResponseBytes);
}

export function decodeRetrieveContextV2ResponseForPayload(
  payload: unknown,
  ...
  literal(root.contract_version, CONTEXT_RETRIEVAL_CONTRACT_V2, "response.contract_version");
  literal(root.ranking_policy, CONTEXT_RETRIEVAL_RANKING_POLICY_V2, "response.ranking_policy");
  ...
  for (const candidate of candidates) {
    ...
    for (const contribution of candidate.contributions) {
      ...
      const reconstructed = retrievalV2ContributionScorePicos(
...
export function decodeRetrieveContextV2ResponseBytes(
  ...
  const payload = decodeContextRetrievalV2Json(bytes, RESPONSE_INTEGER_PATHS);
  return decodeRetrieveContextV2ResponseForPayload(payload, requestPayload, capability, bytes.byteLength);
}

/** Synchronous structural/pin validation only; no cryptographic verification. */ export function assertContextRetrievalCapabilityV2(
...
/** Synchronous structural decoding only; no cryptographic verification. */ export function decodeContextRetrievalCapabilityV2(value: unknown): ContextRetrievalCapabilityV2 {
  try {
...
export function decodeContextRetrievalCapabilityV2Bytes(body: Uint8Array | string): ContextRetrievalCapabilityV2 {
  return decodeContextRetrievalCapabilityV2(decodeContextRetrievalV2Json(body, CAPABILITY_INTEGER_PATHS));
}

export function decodeContextRetrievalCapabilitiesResponseBytes(body: Uint8Array | string): JsonObject {
  const value = decodeContextRetrievalV2Json(body, CAPABILITIES_RESPONSE_INTEGER_PATHS);
...
export function validateContextRetrievalPreflightV2(
  request: RetrieveContextV2Input,
  capability: ContextRetrievalCapabilityV2,
  required: RequiredContextRetrievalCapabilityV2,
): void {
...
function validateContextRetrievalPreflight(
  requestValue: RetrieveContextV2Input,
  capability: ContextRetrievalCapabilityV2,
  required: RequiredContextRetrievalCapabilityV2,
): void {
  ...
  if (request.contractVersion !== attested.contract_version ||
    attested.ranking_policy !== CONTEXT_RETRIEVAL_RANKING_POLICY_V2 ||
...
function decodeAppliedBounds(value: unknown, requested: JsonValue | undefined): RetrievalV2AppliedBounds {
...
function decodeCandidate(value: unknown, index: number): RetrievalV2Candidate {
  ...
  let scores: ReturnType<typeof retrievalV2PreferenceScores>;
  try {
    scores = retrievalV2PreferenceScores(
      baseScorePicos,
...
function decodeContribution(value: unknown, path: string): RetrievalV2Contribution {
...
function decodeNeighbor(value: unknown, path: string, sourceKey: string): RetrievalV2Neighbor {
...
function decodeProviderOutcome(value: unknown, index: number): RetrievalV2ProviderOutcome {
...
function providerDegradationReasons(
  lanes: readonly RetrievalV2CapabilityProviderLane[],
  outcomes: ReadonlyMap<string, RetrievalV2ProviderOutcome>,
): readonly RetrievalV2DegradationReasonCode[] {
  const reasons = new Set<RetrievalV2DegradationReasonCode>();
...
function decodeCapability(value: unknown, path: string): ContextRetrievalCapabilityV2 {
  const input = exactObject(value, [
  ...
  literal(input.contract_version, CONTEXT_RETRIEVAL_CONTRACT_V2, `${path}.contract_version`);
  literal(input.ranking_policy, CONTEXT_RETRIEVAL_RANKING_POLICY_V2, `${path}.ranking_policy`);
  ...
  return freeze({
    ...
    contract_version: CONTEXT_RETRIEVAL_CONTRACT_V2, ranking_policy: CONTEXT_RETRIEVAL_RANKING_POLICY_V2,
...
function decodeRankingParameters(value: unknown, path: string): RetrievalV2RankingParameters {
  const keys = Object.keys(RANKING_PARAMETERS) as (keyof RetrievalV2RankingParameters)[];
  const input = exactObject(value, keys, path);
  for (const key of keys) {
    if (input[key] !== RANKING_PARAMETERS[key]) capabilityFail(`${path}.${key} does not match Retrieval V2`);
  }
  return RANKING_PARAMETERS;
}

function decodeCapabilityLane(value: unknown, path: string): RetrievalV2CapabilityProviderLane {
  const input = exactObject(value, [
...
function decodeCapabilityBounds(value: unknown, path: string): RetrievalV2CapabilityBounds {
  ...
  for (const key of CAPABILITY_BOUND_KEYS) {
    ...
    if (pair.length !== 2 || pair[0] !== specs[key][0] || pair[1] !== specs[key][1]) {
      capabilityFail(`${path}.${key} does not attest the Retrieval V2 mechanical bound`);
    }
    output[key] = freeze([specs[key][0], specs[key][1]]);
  }
  return freeze(output) as unknown as RetrievalV2CapabilityBounds;
}

function compareContributions(left: RetrievalV2Contribution, right: RetrievalV2Contribution): number {
...
function validateNeighbors(candidate: RetrievalV2Candidate, radius: number): void {
