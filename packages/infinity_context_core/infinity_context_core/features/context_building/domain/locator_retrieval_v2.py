"""Deterministic, locator-only domain contract for Retrieval V2."""
...
from infinity_context_core.features.context_building.domain.retrieval_v2_capability import (
    LOCATOR_RETRIEVAL_RANK_CONSTANT_V2 as _LOCATOR_RETRIEVAL_RANK_CONSTANT_V2,
)
from infinity_context_core.features.context_building.domain.retrieval_v2_capability import (
    LOCATOR_RETRIEVAL_RANKING_POLICY_V2,
    LOCATOR_RETRIEVAL_SCORE_SCALE_PICOS_V2,
    LOCATOR_RETRIEVAL_WEIGHT_SCALE_MICROS_V2,
...
LOCATOR_RETRIEVAL_RANK_CONSTANT_V2 = _LOCATOR_RETRIEVAL_RANK_CONSTANT_V2
MAX_QUERY_VARIANTS_V2 = 6
MAX_PROVIDER_REGISTRATIONS_V2 = 4
MAX_PROVIDER_RANK_V2 = 1_000
MAX_RESULT_LIMIT_V2 = 50
MAX_CANDIDATE_LIMIT_V2 = 1_000
MAX_NEIGHBOR_RADIUS_V2 = 2
MAX_DEADLINE_MS_V2 = 2_000
MAX_RESPONSE_BYTES_V2 = 1_048_576
MIN_RESPONSE_BYTES_V2 = 16_384
...
class CanonicalHydrationInvariantErrorV2(RuntimeError):
    """Canonical adapter violated the declared single-snapshot hydration contract."""


@dataclass(frozen=True, slots=True)
class LocatorRetrievalScopeV2:
...
@dataclass(frozen=True, slots=True)
class LocatorQueryVariantV2:
    query_id: str
    query: str
    weight_micros: int = LOCATOR_RETRIEVAL_WEIGHT_SCALE_MICROS_V2
    ...
    def weight(self) -> float:
        ...
        return self.weight_micros / LOCATOR_RETRIEVAL_WEIGHT_SCALE_MICROS_V2


@dataclass(frozen=True, slots=True)
class LocatorRetrievalBoundsV2:
    candidate_limit: int = 150
    result_limit: int = 20
    neighbor_radius: int = 0
    response_byte_limit: int = MAX_RESPONSE_BYTES_V2
    deadline_ms: int = MAX_DEADLINE_MS_V2
    ...
    def __post_init__(self) -> None:
        ...
        if not 1 <= self.candidate_limit <= MAX_CANDIDATE_LIMIT_V2:
            raise ValueError("Locator candidate_limit must be within 1..1000")
        if not 1 <= self.result_limit <= MAX_RESULT_LIMIT_V2:
        ...
        if not 0 <= self.neighbor_radius <= MAX_NEIGHBOR_RADIUS_V2:
            raise ValueError("Locator neighbor_radius must be within 0..2")
        if not MIN_RESPONSE_BYTES_V2 <= self.response_byte_limit <= MAX_RESPONSE_BYTES_V2:
            raise ValueError("Locator response_byte_limit must be within 16384..1048576")
        if not 1 <= self.deadline_ms <= MAX_DEADLINE_MS_V2:
            raise ValueError("Locator deadline_ms must be within 1..2000")


@dataclass(frozen=True, slots=True)
class LocatorRetrievalRequestV2:
    contract_version: str
    capability_fingerprint: str
    profile_id: str
    scope: LocatorRetrievalScopeV2
    queries: tuple[LocatorQueryVariantV2, ...]
    hard_filters: LocatorHardFiltersV2
    soft_preferences: LocatorSoftPreferencesV2
...
class LocatorProviderHitV2:
    ...
    def __post_init__(self) -> None:
        ...
        if not 1 <= self.provider_rank <= MAX_PROVIDER_RANK_V2:
...
@dataclass(frozen=True, slots=True)
class LocatorProviderResultV2:
    hits: tuple[LocatorProviderHitV2, ...] = ()
    status: str = "available"
    reason_code: str | None = None

    def __post_init__(self) -> None:
        raw_hits = _immutable_tuple(self.hits, LocatorProviderHitV2, "provider hits")
        hits = tuple(
...
@dataclass(frozen=True, slots=True)
class CanonicalLocatorCandidateV2:
    ...
    def __post_init__(self) -> None:
        ...
        if self.start_at is not None and self.end_at is not None:
            LocatorTimeIntervalV2(self.start_at, self.end_at)
        ...
        if self.relative_start_ms is not None and self.relative_end_ms is not None:
            LocatorRelativeTimeIntervalV2(self.relative_start_ms, self.relative_end_ms)
...
@dataclass(frozen=True, slots=True)
class CanonicalLocatorReadV2:
    """Seeds and neighbors hydrated in one canonical read transaction/snapshot."""

    seeds: tuple[CanonicalLocatorCandidateV2, ...]
    neighbors: tuple[CanonicalLocatorCandidateV2, ...] = ()

    def __post_init__(self) -> None:
        raw_seeds = _immutable_tuple(self.seeds, CanonicalLocatorCandidateV2, "read seeds")
        raw_neighbors = _immutable_tuple(
            self.neighbors, CanonicalLocatorCandidateV2, "read neighbors"
        )
...
@dataclass(frozen=True, slots=True)
class LocatorScoreContributionV2:
    provider_id: str
    query_id: str
    provider_rank: int
    ...
    def provider_weight(self) -> float:
        return self.provider_weight_micros / LOCATOR_RETRIEVAL_WEIGHT_SCALE_MICROS_V2

    @property
    def query_weight(self) -> float:
        return self.query_weight_micros / LOCATOR_RETRIEVAL_WEIGHT_SCALE_MICROS_V2

    @property
    def contribution(self) -> float:
        return self.contribution_score_picos / LOCATOR_RETRIEVAL_SCORE_SCALE_PICOS_V2


@dataclass(frozen=True, slots=True)
class LocatorNeighborV2:
    locator: str
    source_key: str
    document_key: str
    chunk_key: str
...
@dataclass(frozen=True, slots=True)
class LocatorResultCandidateV2:
    locator: str
    source_key: str
    document_key: str
    chunk_key: str
    ...
    neighbors: tuple[LocatorNeighborV2, ...] = ()
    relation: str = "direct"
    distance: int = 0
    ...
    def __post_init__(self) -> None:
        ...
        raw_contributions = _immutable_tuple(
            self.contributions, LocatorScoreContributionV2, "contributions"
        )
        contributions = tuple(
        ...
        raw_neighbors = _immutable_tuple(self.neighbors, LocatorNeighborV2, "neighbors")
        neighbors = tuple(
            LocatorNeighborV2(
                item.locator,
                item.source_key,
        ...
        if self.fused_score != self.base_score_picos / LOCATOR_RETRIEVAL_SCORE_SCALE_PICOS_V2:
        ...
        expected_preference = (
            0 if requested == 0 else matched_weight * PREFERENCE_SCORE_SCALE_V2 // requested
        )
        ...
        for name in ("preference_score_micros", "preference_boost_micros"):
            _require_int(name, getattr(self, name))
        if not 0 <= self.preference_score_micros <= PREFERENCE_SCORE_SCALE_V2:
            raise ValueError("Ranked locator preference_score_micros is out of bounds")
        expected_boost = (
            self.preference_score_micros
            * MAX_PREFERENCE_BOOST_MICROS_V2
            // PREFERENCE_SCORE_SCALE_V2
        )
        ...
        expected_rerank = (
            base_score_picos
            * (PREFERENCE_SCORE_SCALE_V2 + self.preference_boost_micros)
            // PREFERENCE_SCORE_SCALE_V2
        )
        ...
        if self.matched_query_ids != tuple(
            sorted(set(self.matched_query_ids), key=canonical_string_sort_key_v2)
        ):
        ...
        if contribution_keys != tuple(
            sorted(
                contribution_keys,
                key=lambda item: (
                    canonical_string_sort_key_v2(item[0]),
                    canonical_string_sort_key_v2(item[1]),
                ),
            )
        ):
        ...
        if self.matched_query_ids != tuple(
            sorted(
                ...
                key=canonical_string_sort_key_v2,
            )
        ):
...
@dataclass(frozen=True, slots=True)
class LocatorProviderOutcomeV2:
    provider_id: str
    status: str
    reason_code: str | None = None
...
@dataclass(frozen=True, slots=True)
class LocatorRetrievalResponseV2:
    status: str
    capability_fingerprint: str
    profile_id: str
    applied_bounds: LocatorAppliedBoundsV2
    candidates: tuple[LocatorResultCandidateV2, ...]
    provider_outcomes: tuple[LocatorProviderOutcomeV2, ...]
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.applied_bounds, LocatorAppliedBoundsV2):
        ...
        raw_candidates = _immutable_tuple(
            self.candidates, LocatorResultCandidateV2, "response candidates"
        )
        candidates = tuple(
        ...
        raw_outcomes = _immutable_tuple(
            self.provider_outcomes, LocatorProviderOutcomeV2, "provider outcomes"
        )
        outcomes = tuple(
            LocatorProviderOutcomeV2(item.provider_id, item.status, item.reason_code)
        ...
        if len(self.provider_outcomes) > MAX_PROVIDER_REGISTRATIONS_V2:
        ...
        if self.degradation_reason_codes != tuple(
            sorted(set(self.degradation_reason_codes), key=canonical_string_sort_key_v2)
        ):
        ...
        if ranking_keys != tuple(
            sorted(
                ranking_keys,
                key=lambda item: (item[0], item[1], canonical_string_sort_key_v2(item[2])),
            )
        ):
            raise ValueError(
        ...
        if provider_ids != tuple(sorted(set(provider_ids), key=canonical_string_sort_key_v2)):
...
def candidate_matches_request_v2(
    candidate: CanonicalLocatorCandidateV2,
...
def _copy_canonical(item: CanonicalLocatorCandidateV2) -> CanonicalLocatorCandidateV2:
