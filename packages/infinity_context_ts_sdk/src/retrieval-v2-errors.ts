...
import { assertUnicodeScalarString, pythonTrim } from "./retrieval-v2-canonical.js";
import { decodeContextRetrievalV2Json } from "./retrieval-v2-json.js";
import type { JsonObject } from "./types.js";

export const CONTEXT_RETRIEVAL_V2_ERROR_SPECS = Object.freeze({
...
export type ContextRetrievalV2ErrorCode = keyof typeof CONTEXT_RETRIEVAL_V2_ERROR_SPECS;

export function contextRetrievalV2ErrorDecoder(maximumBytes: number): HttpErrorDecoder {
  if (!Number.isSafeInteger(maximumBytes) || maximumBytes < 0) {
    throw new TypeError("Contract C error byte limit must be a non-negative safe integer");
  }
  return (statusCode, headers, body) => decodeContextRetrievalV2Error(
    statusCode,
    body,
    maximumBytes,
...
export function decodeContextRetrievalV2Error(
  statusCode: number,
  ...
  const root = exactObject(decodeContextRetrievalV2Json(body), ["error"], "error envelope");
  const error = exactObject(root.error, ["code", "message", "retryable"], "error");
  if (typeof error.code !== "string" || !Object.hasOwn(CONTEXT_RETRIEVAL_V2_ERROR_SPECS, error.code)) {
    invalid("error.code is unsupported");
  }
  const code = error.code as ContextRetrievalV2ErrorCode;
  const spec = CONTEXT_RETRIEVAL_V2_ERROR_SPECS[code];
