...
describe("Contract C error envelopes", () => {
  it("behaviorally validates every errors.json status/code/retryability case", async () => {
    ...
    for (const item of fixture.cases) {
      ...
      const error = decodeContextRetrievalV2Error(item.http_status, body);
      expect(error).toMatchObject({
        statusCode: item.http_status, code: item.code, retryable: item.retryable,
      });
      expect(CONTEXT_RETRIEVAL_V2_ERROR_SPECS[item.code as ContextRetrievalV2ErrorCode])
        .toEqual([item.http_status, item.retryable]);
    }
    expect(decodeContextRetrievalV2Error(400, JSON.stringify(fixture.envelope))).toMatchObject({
  ...
  it.each([
    ...
    expect(() => decodeContextRetrievalV2Error(status, body)).toThrow();
  });

  it("rejects malformed UTF-8 and bodies over the caller limit", () => {
    expect(() => decodeContextRetrievalV2Error(400, new Uint8Array([0xff]))).toThrow();
    expect(() => decodeContextRetrievalV2Error(400,
