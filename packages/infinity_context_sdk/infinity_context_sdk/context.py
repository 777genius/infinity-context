...
def _context_item_from_payload(payload: Mapping[str, object]) -> ContextItem:
    ...
    citations = (
        tuple(_citation_from_payload(citation) for citation in citation_payloads[:MAX_SOURCE_REFS])
        if citation_payloads
        else tuple(
            _citation_from_evidence_payload(evidence, item_type=item_type, index=index)
    ...
    return ContextItem(
        ...
        source_refs=tuple(
            _source_ref_from_payload(ref) for ref in source_ref_payloads[:MAX_SOURCE_REFS]
        ),
        citations=citations,
