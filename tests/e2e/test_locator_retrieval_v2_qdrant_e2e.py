"""Opt-in real-Qdrant acceptance for the Retrieval V2 locator profile."""
...
async def _run_large_profile(postgres_url: str) -> None:
    ...
    try:
        ...
        checkpointed = await _fresh_process_lifecycle(
            "attest_checkpoint", database.app_url, identity, process_generation=uuid4().hex,
        ...
        assert int(resumed["resumed_from_item_count"]) == int(
            checkpointed["checkpoint_item_count"]
        )
        ...
        successor_proof = await _fresh_process_lifecycle(
            "build_activate", database.app_url, successor, process_generation=uuid4().hex,
            acknowledge_owner=projection.runtime_owner,
        )
...
async def _fresh_process_lifecycle(
    ...
    *,
    ...
    process = subprocess.Popen(
        [sys.executable, "tests/e2e/locator_retrieval_v2_fresh_process.py"],
    ...
    try:
        ...
        if process.returncode != 0:
            raise AssertionError(f"fresh Retrieval V2 process failed: {stderr[-2000:]}")
...
def _request() -> core.LocatorRetrievalRequestV2:
    return core.LocatorRetrievalRequestV2(
        ...
        scope=core.LocatorRetrievalScopeV2("space-a", "scope-a", "thread-a"),
        queries=(core.LocatorQueryVariantV2("q1", "synthetic decision"),),
        hard_filters=core.LocatorHardFiltersV2(
            source_generations=(
                core.LocatorSourceGenerationV2("source:synthetic", "projection-v1"),
            )
        ),
        soft_preferences=core.LocatorSoftPreferencesV2(),
        bounds=core.LocatorRetrievalBoundsV2(candidate_limit=5, result_limit=5),
    )
