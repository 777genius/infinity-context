...
from infinity_context_core.features.context_building.domain.locator_retrieval import (
    CanonicalLocatorCandidate, CanonicalLocatorRead, LocatorProviderHit,
    LocatorProviderResult, LocatorRetrievalCapability,
    LocatorHardFilters,
    LocatorQueryVariant,
...
from infinity_context_core.features.context_building.application.locator_retrieval import (
    _validated_request_copy, RetrieveLocators, LocatorProviderRegistration,
)
...
def request(thread_id=None, mode="exact"):
    return LocatorRetrievalRequest(
        "context-retrieval.v2", "a" * 64, "profile",
        ...
        LocatorSoftPreferences(), LocatorRetrievalBounds(),
    )


def candidate(thread_id):
    return CanonicalLocatorCandidate(
        locator="locator", canonical_identity="identity", canonical_version=1,
        lifecycle_status="active", space_id="space", memory_scope_id="room",
        source_key="source", document_key="document", chunk_key="chunk",
        projection_generation="generation", kind="turn", category="human",
        read_snapshot="snapshot", thread_id=thread_id,
    )
...
class ThreadSelectorTests(unittest.TestCase):
    ...
    def test_any_retains_canonical_fences(self):
        ...
        for changes in (
            {"space_id": "other"}, {"memory_scope_id": "other-room"},
            {"projection_generation": "stale"}, {"source_key": "excluded"},
            {"source_key": "unadmitted"}, {"lifecycle_status": "deleted"},
        ):
            with self.subTest(changes=changes):
                self.assertFalse(candidate_matches_request(
                    replace(admitted, **changes), request(mode="any")
                ))
    ...
    def test_one_global_engine_call_with_neighbors_and_final_hydration_fences(self):
        def row(key, thread, ordinal=10, **changes):
            return replace(candidate(thread), canonical_identity=key, locator=key,
                           chunk_key=key, sequence_ordinal=ordinal, **changes)
        rows = [row("a", "meeting-a"), row("b", "meeting-b", source_key="source-b", projection_generation="generation-b"), row("null", None),
        ...
        neighbors = (row("a-next", "meeting-a", 11), row("b-next", "meeting-b", 11, source_key="source-b", projection_generation="generation-b"),
                     row("null-next", None, 11), row("bad-gen", "meeting-a", 9,
                                                   projection_generation="stale"))
        for mode, thread, expected in (("any", None, ["a", "b", "null"]),
                                       ("exact", None, ["null"]),
                                       ("exact", "meeting-a", ["a"])):
            calls = []
            ...
            class Provider:
                async def retrieve_locator_candidates(self, req):
                    calls.append(req)
                    return LocatorProviderResult(tuple(
            ...
            class Reader:
                ...
                async def hydrate_final_locator_read(self, req, identities, radius):
                    return CanonicalLocatorRead(
                        tuple(item for item in rows if item.canonical_identity in identities), neighbors
                    )

            engine = RetrieveLocators(
                (LocatorProviderRegistration("lane", Provider(), required=True),), Reader(),
                LocatorRetrievalCapability("a" * 64, "profile", supports_neighbors=True),
            )
            req = request(thread, mode)
            req = replace(req, bounds=replace(req.bounds, neighbor_radius=1),
                          hard_filters=replace(req.hard_filters, source_generations=(
            ...
            self.assertEqual([[n.canonical_identity for n in item.neighbors]
                              for item in result.candidates], [[key + "-next"] for key in expected])
