"""Provider-neutral, deterministic evaluation for locator-only Retrieval."""
...
RETRIEVAL_DATASET_SCHEMA = "generic-retrieval-v2-dataset.v1"
...
class RetrievalEvaluationDataset:
    ...
    schema_version: str = RETRIEVAL_DATASET_SCHEMA
    records_payload: tuple[tuple[str, JsonValue], ...] = ()
    neighbor_pairs: tuple[tuple[str, str], ...] = ()

    def __post_init__(self) -> None:
        _bounded_opaque("dataset_id", self.dataset_id)
        if self.schema_version != RETRIEVAL_DATASET_SCHEMA:
            raise ValueError(f"schema_version must be {RETRIEVAL_DATASET_SCHEMA}")
