"""Provider-free contracts for locator-only Retrieval V2."""
...
@dataclass
class _Provider:
    hits: tuple[LocatorProviderHitV2, ...] = ()
    ...
    async def retrieve_locator_candidates(
        ...
        if self.error is not None:
            raise self.error
        return LocatorProviderResultV2(hits=self.hits)


@dataclass
class _Hydrator:
    candidates: tuple[CanonicalLocatorCandidateV2, ...]
    neighbors: tuple[CanonicalLocatorCandidateV2, ...] = ()
    final_candidates: tuple[CanonicalLocatorCandidateV2, ...] | None = None

    async def hydrate_locator_candidates(
        self,
        _request: LocatorRetrievalRequestV2,
        ...
        return tuple(
            ...
            if not isinstance(item, CanonicalLocatorCandidateV2)
    ...
    async def hydrate_final_locator_read(
        ...
        return CanonicalLocatorReadV2(seeds, self.neighbors)
...
def test_request_bounds_and_unknown_signals_fail_closed() -> None:
    ...
    with pytest.raises(ValueError, match="neighbor_radius"):
        _request(bounds=LocatorRetrievalBoundsV2(neighbor_radius=3))
    with pytest.raises(ValueError, match="response_byte_limit"):
        _request(bounds=LocatorRetrievalBoundsV2(response_byte_limit=0))
    with pytest.raises(ValueError, match="response_byte_limit"):
        _request(bounds=LocatorRetrievalBoundsV2(response_byte_limit=16_383))
...
def test_capability_collections_and_bounds_are_deeply_detached() -> None:
    default = LocatorRetrievalCapabilityV2(FINGERPRINT, "profile")
    ...
    lanes = [
        LocatorProviderLaneCapabilityV2("postgres", True, True, 1_000_000, True),
        LocatorProviderLaneCapabilityV2("qdrant", True, True, 1_250_000, True),
    ]
    required = ["postgres", "qdrant"]
    full = LocatorRetrievalCapabilityV2(
        FINGERPRINT,
        "profile",
...
def test_provider_weight_micros_are_converted_only_for_rrf_execution() -> None:
    ...
    response = asyncio.run(
        _retrieve(
            (LocatorProviderRegistrationV2("dense", provider, weight_micros=1_250_000),),
...
def test_weighted_rrf_is_deterministic_under_provider_hit_and_registration_shuffle() -> None:
    ...
    for seed in range(100):
        ...
        registrations = [
            LocatorProviderRegistrationV2("dense", dense),
            LocatorProviderRegistrationV2("lexical", lexical),
        ]
            ...
            _retrieve(
                ...
                _request(
                    ...
                    preferences=LocatorSoftPreferencesV2(
                        actor_preferences=(LocatorWeightedKeyV2("actor-ok", 1_000_000),)
                    ),
                )
            )
        )
...
def test_no_preferences_preserve_base_order_and_zero_rerank_evidence() -> None:
    response = asyncio.run(
        _retrieve(
            (LocatorProviderRegistrationV2("dense", _Provider((_hit("b"), _hit("a")))),),
...
def test_source_preference_match_reranks_but_miss_does_not() -> None:
    request = _request(
        filters=LocatorHardFiltersV2(
            (
                LocatorSourceGenerationV2("source-ok", "generation-2"),
                LocatorSourceGenerationV2("source-preferred", "generation-2"),
            )
        ),
        preferences=LocatorSoftPreferencesV2(
            source_preferences=(LocatorWeightedKeyV2("source-preferred", 1_000_000),)
        ),
    )
    response = asyncio.run(
        _retrieve(
...
def test_actor_preferences_sum_all_requested_matches_for_multi_actor_candidate() -> None:
    request = _request(
        preferences=LocatorSoftPreferencesV2(
            actor_preferences=(
                LocatorWeightedKeyV2("actor-a", 1_000_000),
                LocatorWeightedKeyV2("actor-b", 3_000_000),
                LocatorWeightedKeyV2("actor-miss", 4_000_000),
            )
        )
    )
    response = asyncio.run(
        _retrieve(
            (LocatorProviderRegistrationV2("dense", _Provider((_hit("multi"),))),),
...
@pytest.mark.parametrize(
    ...
    (
        (
            LocatorSoftPreferencesV2(
                time_interval=LocatorTimeIntervalV2(
        ...
        (
            LocatorSoftPreferencesV2(
                relative_time_interval=LocatorRelativeTimeIntervalV2(10, 20),
                time_weight_micros=1_000_000,
            ),
...
def test_combined_extreme_preferences_are_bounded_and_reconstruct_exactly() -> None:
    preferences = LocatorSoftPreferencesV2(
        source_preferences=(LocatorWeightedKeyV2("source-ok", 10_000_000),),
        actor_preferences=(LocatorWeightedKeyV2("actor-ok", 100_000),),
        relative_time_interval=LocatorRelativeTimeIntervalV2(10, 20),
        time_weight_micros=10_000_000,
    )
    response = asyncio.run(
        _retrieve(
            (LocatorProviderRegistrationV2("dense", _Provider((_hit("a"),))),),
...
def test_inadmissible_candidates_never_receive_preference_evidence() -> None:
    request = _request(
        preferences=LocatorSoftPreferencesV2(
            source_preferences=(LocatorWeightedKeyV2("source-ok", 1_000_000),)
        )
    )
    candidates = (
        _canonical("ok"),
...
def test_hard_filters_are_reapplied_after_canonical_hydration() -> None:
    request = _request(
        filters=LocatorHardFiltersV2(
            source_generations=(LocatorSourceGenerationV2("source-ok", "generation-2"),),
            ...
            tags_none=("private",),
            actor_keys=("actor-ok",),
            time_interval=LocatorTimeIntervalV2(_time(10), _time(20)),
        )
    )
    identities = (
        "ok",
    ...
    response = asyncio.run(
        _retrieve(
            (LocatorProviderRegistrationV2("dense", provider),), _Hydrator(candidates)
        ).execute(request)
    )
...
def test_pair_membership_is_exact_and_never_cartesian() -> None:
    request = _request(
        filters=LocatorHardFiltersV2(
            (
                LocatorSourceGenerationV2("source-a", "generation-a"),
                LocatorSourceGenerationV2("source-b", "generation-b"),
            )
        )
    )
    candidates = (
...
def test_neighbors_attach_after_seed_ranking_and_require_same_source_exact_adjacency() -> None:
    ...
    response = asyncio.run(
        _retrieve(
            (LocatorProviderRegistrationV2("dense", provider),),
            reader,
            supports_neighbors=True,
        ).execute(_request(bounds=LocatorRetrievalBoundsV2(result_limit=2, neighbor_radius=1)))
    )
...
def test_cancellation_is_never_converted_to_degradation() -> None:
    class CancellingProvider:
        async def retrieve_locator_candidates(
            self, _request: LocatorRetrievalRequestV2
        ) -> LocatorProviderResultV2:
    ...
    use_case = _retrieve(
        (
            LocatorProviderRegistrationV2("blocking", blocker),
            LocatorProviderRegistrationV2("cancelling", CancellingProvider()),
        ),
        _Hydrator(()),
    )
...
def test_response_byte_bound_fails_closed_without_partial_candidates() -> None:
    ...
    response = asyncio.run(
        _retrieve(
            ...
            _request(
                bounds=LocatorRetrievalBoundsV2(
...
def test_minimum_byte_limit_holds_maximal_mandatory_error_envelope() -> None:
    ...
    request = LocatorRetrievalRequestV2(
        ...
        profile_id,
        LocatorRetrievalScopeV2("space", "scope"),
        (_query("q1"),),
        LocatorHardFiltersV2((LocatorSourceGenerationV2("source-ok", "generation-2"),)),
        LocatorSoftPreferencesV2(),
        LocatorRetrievalBoundsV2(response_byte_limit=16_384),
    )
    response = asyncio.run(
        RetrieveLocatorsV2(
            providers,
            _Hydrator(()),
            LocatorRetrievalCapabilityV2(capability_fingerprint, profile_id),
        ).execute(request)
    )
    encoded = json.dumps(
        asdict(response), ensure_ascii=False, separators=(",", ":"), sort_keys=True
...
def test_mutable_requests_registrations_and_provider_results_are_defensively_copied() -> None:
    ...
    hits = [_hit("a")]
    result = LocatorProviderResultV2(hits=hits)  # type: ignore[arg-type]
    hits.append(_hit("b"))
    assert tuple(item.canonical_identity for item in result.hits) == ("a",)

    registrations = [LocatorProviderRegistrationV2("dense", _Provider(result.hits))]
...
def test_request_is_revalidated_at_use_case_entry() -> None:
    ...
    with pytest.raises(ValueError, match="1..6"):
        asyncio.run(
            _retrieve(
                (LocatorProviderRegistrationV2("dense", _Provider()),), _Hydrator(())
            ).execute(request)
        )
...
def test_malformed_duplicate_or_mixed_preliminary_hydration_fails_typed(
    ...
    with pytest.raises(CanonicalHydrationInvariantErrorV2):
        asyncio.run(
            _retrieve((LocatorProviderRegistrationV2("dense", provider),), reader).execute(
                _request()
            )
        )
...
def test_final_read_mixed_snapshots_fails_typed() -> None:
    ...
    with pytest.raises(CanonicalHydrationInvariantErrorV2, match="mixed read snapshots"):
        asyncio.run(
            _retrieve(
                (LocatorProviderRegistrationV2("dense", _Provider((_hit("a"),))),),
                reader,
                supports_neighbors=True,
            ).execute(_request(bounds=LocatorRetrievalBoundsV2(neighbor_radius=1)))
        )
...
def test_unattested_neighbors_and_profile_mismatch_return_before_provider_calls() -> None:
    ...
    use_case = _retrieve(
        (LocatorProviderRegistrationV2("dense", provider),), _Hydrator((_canonical("a"),))
    )
    unsupported = asyncio.run(
        use_case.execute(_request(bounds=LocatorRetrievalBoundsV2(neighbor_radius=1)))
    )
...
def test_full_capability_requires_exact_healthy_qualified_provider_membership() -> None:
    lanes = (
        LocatorProviderLaneCapabilityV2("postgres_keyword", True, True, 1_000_000, True),
        LocatorProviderLaneCapabilityV2("qdrant_dense", True, True, 1_000_000, True),
    )
    capability = LocatorRetrievalCapabilityV2(
    ...
    accepted = asyncio.run(
        RetrieveLocatorsV2(providers, _Hydrator((_canonical("a"),)), capability).execute(request)
    )
    assert accepted.status == "available"

    unhealthy = LocatorRetrievalCapabilityV2(
        ...
        (
            lanes[0],
            LocatorProviderLaneCapabilityV2("qdrant_dense", True, False, 1_000_000, True),
        ),
        required_provider_lanes=("postgres_keyword", "qdrant_dense"),
    )
    rejected = asyncio.run(
        RetrieveLocatorsV2(providers, _Hydrator((_canonical("a"),)), unhealthy).execute(request)
    )
...
def _retrieve(
    providers: tuple[LocatorProviderRegistrationV2, ...] | list[LocatorProviderRegistrationV2],
    reader: _Hydrator,
    *,
    supports_neighbors: bool = False,
) -> RetrieveLocatorsV2:
    return RetrieveLocatorsV2(
        providers,  # type: ignore[arg-type]
        reader,
        LocatorRetrievalCapabilityV2(FINGERPRINT, "hybrid-general-v2", supports_neighbors),
    )


def _request(
    *,
    queries: tuple[LocatorQueryVariantV2, ...] | None = None,
    filters: LocatorHardFiltersV2 | None = None,
    preferences: LocatorSoftPreferencesV2 | None = None,
    bounds: LocatorRetrievalBoundsV2 | None = None,
) -> LocatorRetrievalRequestV2:
    return LocatorRetrievalRequestV2(
        contract_version=LOCATOR_RETRIEVAL_CONTRACT_VERSION_V2,
        capability_fingerprint=FINGERPRINT,
        profile_id="hybrid-general-v2",
        scope=LocatorRetrievalScopeV2("space", "scope"),
        queries=(_query("q1"),) if queries is None else queries,
        hard_filters=filters
        or LocatorHardFiltersV2((LocatorSourceGenerationV2("source-ok", "generation-2"),)),
        soft_preferences=preferences or LocatorSoftPreferencesV2(),
        bounds=bounds or LocatorRetrievalBoundsV2(),
    )


def _query(query_id: str, *, weight: float | None = None) -> LocatorQueryVariantV2:
    weight_micros = 1_000_000 if weight is None else round(weight * 1_000_000)
    return LocatorQueryVariantV2(
        query_id=query_id,
...
def _canonical(
    ...
    *,
    ...
    return CanonicalLocatorCandidateV2(
