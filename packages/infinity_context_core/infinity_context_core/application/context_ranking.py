...
import infinity_context_core.application.context_rerank_domain_heuristics as _domain_heuristics
import infinity_context_core.application.context_rerank_evidence_signals as _evidence_signals
import infinity_context_core.application.context_rerank_relevance as _rerank_relevance
...
def _deterministic_rerank_signals(
    ...
    *,
    ...
    query_text, query_reason, relevance = _rerank_relevance.best_query_relevance_for_rerank(
        plan,
        item=item,
        cache=query_relevance_cache,
        best_query_relevance_fn=best_query_relevance,
    )
    del query_text
    ...
    source_speaker_anchor_override = (
        ...
        and _domain_heuristics.dialogue_speaker_confirms_query_anchor(
            item=item,
            query_anchor_intent=query_anchor_intent,
            relevance=relevance,
        )
    ...
    if artifact_inventory_penalty > 0:
        ...
        reasons.append(artifact_inventory_reason)
        rank_signals.update(artifact_inventory_signals)
    owner_boost, owner_penalty, owner_reason = _domain_heuristics.activity_owner_signal(
        query_anchor_intent=query_anchor_intent,
        query_reason=query_reason,
        text=item.text,
    )
    ...
    if (
        ...
        and not _domain_heuristics.is_item_purchase_temporal_answer_evidence(
            query_reason=query_reason,
            item=item,
        )
    ):
    ...
    temporal_answer_boost, temporal_answer_penalty, temporal_answer_reason = (
        _domain_heuristics.temporal_answer_signal(
            query=query,
            query_reason=query_reason,
            item=item,
        )
    ...
    if _domain_heuristics.event_participation_mismatch(query=query, text=item.text):
        penalty += 0.075
        reasons.append("event_participation_mismatch")
    elif _domain_heuristics.event_participation_positive_match(query=query, text=item.text):
        boost += 0.018
        reasons.append("event_participation_positive_match")
    elif _domain_heuristics.event_participation_source_sibling_noise(
        query=query,
        item=item,
    ):
        penalty += 0.07
    ...
    if companion_penalty > 0:
        ...
        reasons.append(companion_reason)
    if _domain_heuristics.activity_source_sibling_noise(item=item):
        penalty += 0.04
        reasons.append("activity_source_sibling_noise")
    if _domain_heuristics.capped_source_sibling_low_signal(item=item):
        penalty += 0.06
        reasons.append("capped_source_sibling_low_signal")
    if _domain_heuristics.allergy_condition_weak_evidence(
        query_reason=query_reason,
        relevance=relevance,
    ):
        penalty += 0.07
        reasons.append("allergy_condition_weak_evidence")
    if _domain_heuristics.patriotic_service_weak_evidence(
        query_reason=query_reason,
        relevance=relevance,
    ):
        penalty += 0.055
        reasons.append("patriotic_service_weak_evidence")
    if _domain_heuristics.running_reason_weak_evidence(
        query_reason=query_reason,
        relevance=relevance,
    ):
        penalty += 0.055
        reasons.append("running_reason_weak_evidence")
    if _domain_heuristics.volunteer_career_exact_turn_evidence(
        query_reason=query_reason,
        item=item,
    ):
        boost += 0.028
        reasons.append("volunteer_career_exact_turn_evidence")
    if _domain_heuristics.volunteer_career_weak_evidence(
        query_anchor_intent=query_anchor_intent,
        query_reason=query_reason,
        item=item,
    ):
    ...
    if _domain_heuristics.volunteer_career_broad_evidence(
        query_reason=query_reason,
        item=item,
    ):
        penalty += 0.07
        reasons.append("volunteer_career_broad_evidence")
    if _domain_heuristics.post_event_activity_timing_exact_evidence(
        query_reason=query_reason,
        item=item,
    ):
        boost += 0.032
        reasons.append("post_event_activity_timing_exact_evidence")
    if _domain_heuristics.post_event_activity_timing_weak_evidence(
        query_reason=query_reason,
        item=item,
    ):
        penalty += 0.12
        reasons.append("post_event_activity_timing_weak_evidence")
    if _domain_heuristics.shoe_usage_exact_evidence(
        query_reason=query_reason,
        item=item,
    ):
        boost += 0.024
        reasons.append("shoe_usage_exact_evidence")
    if _domain_heuristics.shoe_usage_weak_evidence(
        query_reason=query_reason,
        item=item,
    ):
        penalty += 0.12
    ...
    if (
        ...
        _rerank_relevance.is_long_query_weak_overlap(relevance)
        and anchor_match is None
        and coverage_ratio <= 0
