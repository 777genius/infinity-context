...
def test_wire_response_is_locator_only_at_every_depth() -> None:
    ...
    response = core.LocatorRetrievalResponseV2(
        ...
        profile_id="profile",
        applied_bounds=core.LocatorAppliedBoundsV2(10, 1, 1, 16384, 2000, 1, 1),
        candidates=(candidate,),
        provider_outcomes=(core.LocatorProviderOutcomeV2("postgres_keyword", "available"),),
    )
...
def test_slow_incomplete_request_body_is_inside_server_deadline(monkeypatch) -> None:
    ...
    response = asyncio.run(
        context_retrieval.retrieve_context(SlowRequest(), SimpleNamespace())
    )
