"""Strict raw-byte JSON seam for canonical Retrieval V2 contracts."""
...
def decode_context_retrieval_v2_json(raw: bytes) -> Mapping[str, object]:
    ...
    value = _normalize_context_retrieval_v2_unicode(decoded)
    if not isinstance(value, Mapping):
        raise ValueError("canonical JSON input must contain one object")
    return value


def decode_retrieve_context_v2_request(raw: bytes):
    from ._context_building_retrieval_v2 import RetrieveContextV2RequestDto

    return RetrieveContextV2RequestDto.from_dict(decode_context_retrieval_v2_json(raw))


def decode_retrieve_context_v2_response(raw: bytes):
    from ._context_building_retrieval_v2 import RetrieveContextV2ResponseDto

    return RetrieveContextV2ResponseDto.from_dict(decode_context_retrieval_v2_json(raw))


def _normalize_context_retrieval_v2_unicode(value: object) -> object:
    ...
    if isinstance(value, Mapping):
        ...
        for key, nested in value.items():
            ...
            normalized[scalar_key] = _normalize_context_retrieval_v2_unicode(nested)
        return normalized
    if isinstance(value, Sequence) and not isinstance(value, bytes | bytearray):
        return [_normalize_context_retrieval_v2_unicode(item) for item in value]
    return value
