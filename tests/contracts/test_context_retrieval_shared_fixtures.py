...
def test_fixture_set_and_capability_fingerprint_are_canonical() -> None:
    ...
    parsed = RetrievalCapabilityDto.from_dict(capability)

    assert parsed.to_dict() == capability
    assert capability_fingerprint(capability) == (
...
def test_capability_wire_weight_micros_rejects_bool_float_and_range_drift(
    ...
    with pytest.raises(ValueError, match="weight_micros"):
        RetrievalCapabilityDto.from_dict(capability)
...
def test_ranking_parameter_mutation_changes_fingerprint_and_is_rejected(
    ...
    assert capability_fingerprint(capability) != old_fingerprint
    capability["capability_fingerprint"] = capability_fingerprint(capability)
...
def test_utf8_byte_order_matches_independent_node_oracle_for_unicode_and_duplicates() -> None:
    ...
    expected_fingerprint = capability_fingerprint(payload)
    script = r"""
const crypto = require('crypto');
const fs = require('fs');
...
"""
    ...
    RetrievalHardFiltersDto(
        tuple(RetrievalSourceGenerationDto(value, "generation") for value in ordered_unique)
    )
    core_context.LocatorHardFilters(
        tuple(core_context.LocatorSourceGeneration(value, "generation") for value in ordered_unique)
    )
    unicode_outcomes = tuple(
        RetrievalProviderOutcomeDto(value, "available") for value in ("\ue000", "\U00010000")
    )
    RetrieveContextResponseDto(
        "unqualified",
        "a" * 64,
...
def test_request_success_and_projection_fixtures_serialize_exactly() -> None:
    ...
    assert RetrieveContextRequestDto.from_dict(request).to_dict() == request
    assert RetrieveContextResponseDto.from_dict(success).to_dict() == success
...
def test_error_fixture_is_the_exact_framework_neutral_envelope_and_code_table() -> None:
    ...
    envelope = RetrievalErrorEnvelopeDto.from_dict(errors["envelope"])
    ...
    assert actual == CONTEXT_RETRIEVAL_ERROR_SPECS
...
def test_duplicate_locator_and_identity_reach_global_uniqueness_guard(
    ...
    with pytest.raises(ValueError, match="Locator response candidates must be globally unique"):
        RetrieveContextResponseDto.from_dict(success)
...
def _execute_shared_case(case_id: str) -> str:
    ...
    if case_id == "wire_exact_accept":
        RetrieveContextRequestDto.from_dict(request)
        return "accept"
    if case_id == "unknown_or_text_field_reject":
        request["text"] = "forbidden"
        return _typed_rejection(lambda: RetrieveContextRequestDto.from_dict(request))
    if case_id == "capability_shape_or_fingerprint_drift_reject":
        capability = _load("capability.json")
        capability["profile_id"] = "drifted-profile"
        return _typed_rejection(lambda: RetrievalCapabilityDto.from_dict(capability))
    ...
    if case_id == "bounds_changed_or_response_oversize_reject":
        ...
        response = asyncio.run(
            _core_retriever(candidates=candidates).execute(
                _core_request(
                    bounds=core_context.LocatorRetrievalBounds(
                        candidate_limit=50,
                        result_limit=50,
    ...
    if case_id == "duplicate_locator_or_identity_reject":
        ...
        return _typed_rejection(lambda: RetrieveContextResponseDto.from_dict(success))
    ...
    if case_id == "profile_digest_generation_or_membership_drift_unavailable":
        parsed = RetrievalCapabilityDto.from_dict(_load("capability.json"))
        capability = _core_capability(parsed)
        response = asyncio.run(
            _core_retriever(
...
class _LocatorProvider:
    provider_id: str
    candidates: tuple[core_context.CanonicalLocatorCandidate, ...]
    hit_version: int | None = None

    async def retrieve_locator_candidates(
        self, _request: core_context.LocatorRetrievalRequest
    ) -> core_context.LocatorProviderResult:
        return core_context.LocatorProviderResult(
            tuple(
                core_context.LocatorProviderHit(
...
class _CanonicalReader:
    ...
    async def hydrate_locator_candidates(
        self,
        _request: core_context.LocatorRetrievalRequest,
        canonical_identities: tuple[str, ...],
    ) -> tuple[core_context.CanonicalLocatorCandidate, ...]:
    ...
    async def hydrate_final_locator_read(
        ...
        canonical_identities: tuple[str, ...],
        _radius: int,
    ) -> core_context.CanonicalLocatorRead:
        selected = set(canonical_identities)
        return core_context.CanonicalLocatorRead(
...
class _OwnershipFilteringReader(_CanonicalReader):
    ...
    async def hydrate_locator_candidates(
        self,
        request: core_context.LocatorRetrievalRequest,
        canonical_identities: tuple[str, ...],
    ) -> tuple[core_context.CanonicalLocatorCandidate, ...]:
...
def _core_retriever(
    *,
    candidates: tuple[core_context.CanonicalLocatorCandidate, ...],
    capability: core_context.LocatorRetrievalCapability | None = None,
    profile_id: str = "profile",
    provider_ids: tuple[str, ...] | None = None,
    hit_version: int | None = None,
    reader: _CanonicalReader | None = None,
) -> core_context.RetrieveLocators:
    descriptor = capability or core_context.LocatorRetrievalCapability("a" * 64, profile_id)
    lane_ids = tuple(lane.provider_id for lane in descriptor.provider_lanes)
    ids = provider_ids or lane_ids or ("dense",)
    providers = tuple(
        core_context.LocatorProviderRegistration(
            provider_id,
    ...
    return core_context.RetrieveLocators(
...
def _core_request(
    ...
    profile_id: str = "profile",
    *,
    bounds: core_context.LocatorRetrievalBounds | None = None,
) -> core_context.LocatorRetrievalRequest:
    return core_context.LocatorRetrievalRequest(
        core_context.LOCATOR_RETRIEVAL_CONTRACT_VERSION,
        fingerprint,
        profile_id,
        core_context.LocatorRetrievalScope("space", "scope"),
        (core_context.LocatorQueryVariant("q1", "query"),),
        core_context.LocatorHardFilters(
            (core_context.LocatorSourceGeneration("source", "generation"),)
        ),
        core_context.LocatorSoftPreferences(),
        bounds or core_context.LocatorRetrievalBounds(),
    )
...
def _core_candidate(
    ...
    *,
    ...
    version: int = 1,
) -> core_context.CanonicalLocatorCandidate:
    return core_context.CanonicalLocatorCandidate(
...
def _core_capability(
    dto: RetrievalCapabilityDto,
) -> core_context.LocatorRetrievalCapability:
    return core_context.LocatorRetrievalCapability(
        ...
        provider_lanes=tuple(
            core_context.LocatorProviderLaneCapability(
                lane.provider_id,
                lane.required,
                lane.healthy,
                lane.weight_micros,
...
def _success_dto(payload: dict[str, object]) -> RetrieveContextResponseDto:
    bounds = payload["applied_bounds"]
    candidate = payload["candidates"][0]
    assert isinstance(bounds, dict) and isinstance(candidate, dict)
    contributions = tuple(RetrievalContributionDto(**item) for item in candidate["contributions"])
    neighbors = tuple(RetrievalNeighborDto(**item) for item in candidate["neighbors"])
    direct = RetrievalCandidateDto(
        **{
            key: value
    ...
    outcomes = tuple(RetrievalProviderOutcomeDto(**item) for item in payload["provider_outcomes"])
    return RetrieveContextResponseDto(
        status=payload["status"],
        capability_fingerprint=payload["capability_fingerprint"],
        profile_id=payload["profile_id"],
        applied_bounds=RetrievalAppliedBoundsDto(**bounds),
        candidates=(direct,),
...
def _load(name: str) -> dict[str, object]:
    return dict(decode_context_retrieval_json((FIXTURES / name).read_bytes()))
