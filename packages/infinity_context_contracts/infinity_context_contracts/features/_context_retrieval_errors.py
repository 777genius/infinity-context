"""Framework-neutral stable error envelope for Retrieval and projection ingest."""
...
class RetrievalErrorDto:
    code: str
    message: str
    retryable: bool

    def __post_init__(self) -> None:
        spec = CONTEXT_RETRIEVAL_ERROR_SPECS.get(self.code)
        if spec is None:
...
@dataclass(frozen=True, slots=True)
class RetrievalErrorEnvelopeDto:
    error: RetrievalErrorDto

    def __post_init__(self) -> None:
        if not isinstance(self.error, RetrievalErrorDto):
            raise ValueError("error must be a RetrievalErrorDto")

    @property
    def http_status(self) -> int:
        return CONTEXT_RETRIEVAL_ERROR_SPECS[self.error.code][0]

    def to_dict(self) -> JsonObject:
        return {"error": self.error.to_dict()}

    @classmethod
    def from_dict(cls, payload: Mapping[str, object]) -> RetrievalErrorEnvelopeDto:
