"""Canonical filter, interval, and preference DTOs for Retrieval."""
...
class RetrievalTimeIntervalDto:
    start_at: str
    end_at: str
...
@dataclass(frozen=True, slots=True)
class RetrievalRelativeTimeIntervalDto:
    start_ms: int
    end_ms: int
...
class RetrievalSourceGenerationDto:
    source_key: str
...
@dataclass(frozen=True, slots=True)
class RetrievalHardFiltersDto:
    source_generations: Sequence[RetrievalSourceGenerationDto]
    ...
    time_interval: RetrievalTimeIntervalDto | None = None
    relative_time_interval: RetrievalRelativeTimeIntervalDto | None = None
    ...
    def __post_init__(self) -> None:
        ...
        pairs = tuple(
            RetrievalSourceGenerationDto(item.source_key, item.projection_generation)
...
@dataclass(frozen=True, slots=True)
class RetrievalSoftPreferencesDto:
    source_preferences: Sequence[RetrievalWeightedKeyDto] = field(default_factory=tuple)
    actor_preferences: Sequence[RetrievalWeightedKeyDto] = field(default_factory=tuple)
    time_interval: RetrievalTimeIntervalDto | None = None
    relative_time_interval: RetrievalRelativeTimeIntervalDto | None = None
    ...
    def __post_init__(self) -> None:
        for name in ("source_preferences", "actor_preferences"):
            raw = canonical_tuple(
                getattr(self, name), RetrievalWeightedKeyDto, f"soft_preferences.{name}"
            )
            values = tuple(RetrievalWeightedKeyDto(item.key, item.weight_micros) for item in raw)
...
def parse_filters(payload: Mapping[str, object]) -> RetrievalHardFiltersDto:
    allowed = {
        "source_generations",
        "excluded_source_keys",
        "document_keys",
...
def parse_soft_preferences(payload: Mapping[str, object]) -> RetrievalSoftPreferencesDto:
    require_exact(
        payload,
        {
            "source_preferences",
    ...
    return RetrievalSoftPreferencesDto(
...
def parse_optional_time_interval(value: object, path: str) -> RetrievalTimeIntervalDto | None:
    if value is None:
        return None
    ...
    return RetrievalTimeIntervalDto(
...
def parse_optional_relative_interval(
    ...
    return RetrievalRelativeTimeIntervalDto(
...
def _parse_source_generation(
    ...
    return RetrievalSourceGenerationDto(
...
def _validate_interval_types(
    absolute: RetrievalTimeIntervalDto | None,
    relative: RetrievalRelativeTimeIntervalDto | None,
    path: str,
) -> None:
    if absolute is not None and not isinstance(absolute, RetrievalTimeIntervalDto):
        raise ValueError(f"{path}.time_interval has an invalid runtime type")
    if relative is not None and not isinstance(relative, RetrievalRelativeTimeIntervalDto):
