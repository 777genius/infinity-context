...
def test_official_python_client_retrieval_v2_matches_shared_locator_fixture() -> None:
    ...
    result = _client(handler).retrieve_context_v2(request, capability=capability)
...
def test_official_python_client_rejects_unknown_response_fields_and_attestation_drift() -> None:
    ...
    with pytest.raises(InfinityContextRetrievalV2ContractError):
        _client(lambda _request: httpx.Response(200, json=body)).retrieve_context_v2(
    ...
    with pytest.raises(InfinityContextRetrievalV2ContractError):
        _client(lambda _request: httpx.Response(200, json={})).retrieve_context_v2(
            RetrieveContextV2RequestDto.from_dict(changed), capability=capability
        )
...
def test_official_python_client_maps_strict_errors_and_transport_timeout() -> None:
    ...
    error = {
        "error": {
            ...
            "message": "Retrieval V2 is unavailable",
            "retryable": True,
        }
    }
    with pytest.raises(InfinityContextRetrievalV2Error) as raised:
        _client(lambda _request: httpx.Response(503, json=error)).retrieve_context_v2(
            request, capability=capability
        )
    ...
    with pytest.raises(InfinityContextRetrievalV2Error) as timed_out:
        _client(timeout).retrieve_context_v2(request, capability=capability)
    ...
    with pytest.raises(InfinityContextRetrievalV2Error) as network:
        _client(unavailable).retrieve_context_v2(request, capability=capability)
...
def test_official_python_client_rejects_shared_hostile_cross_envelopes() -> None:
    ...
    for case in fixture["cases"]:
        ...
        with pytest.raises(InfinityContextRetrievalV2ContractError, match=".*") as rejected:
            _client(lambda _request, body=body: httpx.Response(200, json=body)).retrieve_context_v2(
                request, capability=capability
            )
...
def test_official_python_client_enforces_absolute_slow_stream_budget_and_cancellation() -> None:
    ...
    with pytest.raises(InfinityContextRetrievalV2Error) as aborted:
        client.retrieve_context_v2(request, capability=capability, cancellation_event=cancelled)
...
def _inputs():
    request = RetrieveContextV2RequestDto.from_dict(
        json.loads((FIXTURES / "request.json").read_text())
    )
    capability = RetrievalV2CapabilityDto.from_dict(
