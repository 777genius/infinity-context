...
def test_query_integrity_checks_selected_query_plan_text(
    tmp_path: Path,
) -> None:
    dataset = tmp_path / "unused.json"
    dataset.write_text("[]", encoding="utf-8")
    case = _case(
        case_id="conv-1:qa:query-plan-leak",
        question="What did Caroline mention?",
        expected_terms=("love", "faith"),
        answer="love and faith",
    )

    class _QueryPlanLeakBackend(_StaticBackend):
        def search(
            self,
            case: PublicBenchmarkCase,
            *,
                ...
                metadata={
                    "query_decomposition": {
                        ...
                        "query_plan": {
                            ...
                            "selected": [
                                {
                                    "role": "relation_compact",
                                    "query": "caroline mention love faith",
                                }
                            ],
                        },
                    },
    ...
    result = run_memory_comparison_benchmark(
        ...
        top_k=1,
        top_k_cutoffs=(1,),
        run_id="query-integrity-plan-unit",
    )
...
def test_memory_comparison_official_locomo_turn_mode_accepts_dialogue_wrapped_session(
    tmp_path: Path,
) -> None:
    dataset = tmp_path / "locomo10-dialogue-wrapped-session.json"
    dataset.write_text(
        json.dumps(
            [
                {
                    "sample_id": "conv-dialogue-wrapped-turns",
                    "conversation": {
                        "speaker_a": "Caroline",
                        "session_1": {
    ...
    assert len(cases) == 1
    assert cases[0].memories[0].source_external_id == (
        "locomo:conv-dialogue-wrapped-turns:session_1:D1:1:turn"
    )
    assert "session_1 date: 1:56 pm on 8 May, 2023" in cases[0].memories[0].text
    assert cases[0].expected_terms == ("blue binder",)
...
def test_query_decomposition_classifies_since_when_as_duration() -> None:
    case = _case(
        case_id="conv-1:qa:since-when-duration",
        question="Since when has Maria lived in Sweden?",
        expected_terms=("since 2021",),
        answer="since 2021",
        category=2,
    )
    ...
    assert metadata["query_profile"]["entity_surfaces"] == ("maria", "sweden")
    assert metadata["query_profile"]["time_intent_kind"] == "duration"
    assert "duration" in metadata["query_profile"]["temporal_terms"]
    assert "duration_temporal_support" in metadata["query_plan"]["selected_roles"]
    assert queries[-1] == (
        "maria sweden when since duration start year month session"
    )
...
def test_query_decomposition_classifies_daypart_relative_temporal_queries() -> None:
    case = _case(
    ...
    assert metadata["query_profile"]["time_intent_kind"] == "relative_time"
    assert "this afternoon" in metadata["query_profile"]["temporal_terms"]
    assert "after" not in metadata["query_profile"]["temporal_terms"]
    assert "relative_temporal_support" in metadata["query_plan"]["selected_roles"]
    assert queries[-1] == "morgan this afternoon session date time"


def test_benchmark_rerank_marks_daypart_relative_time_surface() -> None:
    case = _case(
        case_id="daypart-relative-rerank",
        question="What did Morgan review this afternoon?",
        expected_terms=("studio plan",),
        answer="studio plan",
        category=2,
    ...
    topical = RetrievedMemory(
        ...
        rank=1,
        score=0.05,
        text="D1:1 Morgan: I reviewed the checklist for the studio.",
    )
    daypart_plan = RetrievedMemory(
        item_id="daypart-plan",
        rank=2,
        score=0.0,
    ...
    reranked, metadata = rerank_module.benchmark_rerank_memories(
        case,
        (topical, daypart_plan),
    )

    assert metadata["query_profile"]["time_intent_kind"] == "relative_time"
    assert [memory.item_id for memory in reranked] == ["daypart-plan", "topic"]
    diagnostics = reranked[0].metadata["diagnostics"]
    features = diagnostics["benchmark_candidate_features"]
    assert features["has_temporal_surface"] is True
    assert features["has_relative_time_surface"] is True
...
def test_query_decomposition_expands_location_profile_queries() -> None:
    ...
    based_case = _case(
        case_id="location-profile-based",
        question="Where is Alex based?",
        expected_terms=("Denver",),
        answer="Denver",
    )
    based_city_case = _case(
        case_id="location-profile-based-city",
        question="Which city is Alex based in?",
        expected_terms=("Denver",),
        answer="Denver",
    )
    living_case = _case(
        case_id="location-profile-living",
        question="Where is Alex living?",
        expected_terms=("Denver",),
        answer="Denver",
    )
    current_city_case = _case(
        case_id="location-profile-current-city",
        question="What is Alex's current city?",
        expected_terms=("Denver",),
        answer="Denver",
    )
    origin_case = _case(
    ...
    raised_case = _case(
        case_id="location-profile-raised",
        question="Where was Alex raised?",
        expected_terms=("Toronto",),
        answer="Toronto",
    )
    hometown_case = _case(
    ...
    staying_case = _case(
        case_id="location-profile-staying",
        question="Where is Alex staying?",
        expected_terms=("Hilton",),
        answer="Hilton",
    )
    ...
    based_on_case = _case(
        case_id="location-profile-based-on-guard",
        question="What is Alex based on?",
        expected_terms=("facts",),
        answer="facts",
    )

    live_queries, live_metadata = rerank_module.decomposed_search_queries(live_case)
    based_queries, based_metadata = rerank_module.decomposed_search_queries(
        based_case
    )
    based_city_queries, based_city_metadata = rerank_module.decomposed_search_queries(
        based_city_case
    )
    living_queries, living_metadata = rerank_module.decomposed_search_queries(
        living_case
    )
    current_city_queries, current_city_metadata = (
        rerank_module.decomposed_search_queries(current_city_case)
    )
    ...
    raised_queries, raised_metadata = rerank_module.decomposed_search_queries(
        raised_case
    )
    ...
    staying_queries, staying_metadata = rerank_module.decomposed_search_queries(
        staying_case
    )
    _, stay_present_metadata = rerank_module.decomposed_search_queries(
        stay_present_case
    )
    _, based_on_metadata = rerank_module.decomposed_search_queries(based_on_case)

    assert live_queries[1] == "alex live liv bas home city place"
    assert live_queries[2] == "alex live liv bas home city place from origin country"
    ...
    assert based_queries[1] == "alex live liv bas home city place"
    assert based_queries[2] == "alex live liv bas home city place from origin country"
    assert based_metadata["query_profile"]["relation_terms"] == ("live",)
    assert based_metadata["query_profile"]["evidence_need"] == ("location_support",)
    assert based_city_queries[1] == "alex live liv bas home city place"
    assert based_city_metadata["query_profile"]["relation_terms"] == ("live",)
    assert based_city_metadata["query_profile"]["evidence_need"] == (
        "location_support",
    )
    ...
    assert current_city_metadata["query_profile"]["relation_categories"] == (
        "location_transition",
    )
    assert current_city_metadata["query_profile"]["evidence_need"] == (
        "location_support",
    )
    ...
    assert raised_queries[1] == "alex origin home city country grew born"
    assert raised_queries[2] == "alex origin home city country grew born hometown childhood raise"
    assert raised_metadata["query_profile"]["relation_terms"] == ("origin",)
    assert raised_metadata["query_profile"]["evidence_need"] == ("location_support",)
    ...
    assert staying_queries[1] == "alex stay hotel place conference"
    assert staying_metadata["query_profile"]["relation_terms"] == ("stay",)
    assert staying_metadata["query_profile"]["evidence_need"] == ("location_support",)
    ...
    assert "location_support" not in based_on_metadata["query_profile"][
        "evidence_need"
    ]
...
def test_query_decomposition_expands_health_profile_queries() -> None:
    ...
    physician_case = _case(
        case_id="health-profile-physician",
        question="Who is Alex's primary care physician?",
        expected_terms=("Dr. Lee",),
        answer="Dr. Lee",
    )
    ...
    physician_queries, physician_metadata = rerank_module.decomposed_search_queries(
        physician_case
    )
    ...
    assert doctor_queries[2] == "alex health doctor physician therapist medication medicine"
    ...
    assert physician_queries[2] == (
        "alex health doctor physician therapist medication medicine"
    )
    assert physician_metadata["query_profile"]["relation_categories"] == (
        "health_profile",
    )
    assert physician_metadata["query_profile"]["evidence_need"] == (
        "health_profile",
    )
    assert "physician" in physician_metadata["query_profile"][
        "relation_variant_terms"
    ]
...
def test_query_decomposition_expands_vehicle_profile_queries() -> None:
    ...
    plate_case = _case(
        case_id="vehicle-profile-license-plate",
        question="What is Alex's license plate?",
    ...
    dinner_plate_case = _case(
        case_id="vehicle-profile-plate-guard",
        question="What plate did Alex use for dinner?",
        expected_terms=("blue plate",),
        answer="blue plate",
    )
    road_case = _case(
    ...
    plate_queries, plate_metadata = rerank_module.decomposed_search_queries(plate_case)
    _, travel_metadata = rerank_module.decomposed_search_queries(travel_case)
    _, dinner_plate_metadata = rerank_module.decomposed_search_queries(dinner_plate_case)
    ...
    assert plate_queries[2] == "alex vehicle license licence plate car drive"
    assert plate_metadata["query_profile"]["relation_categories"] == (
        "vehicle_profile",
    )
    assert plate_metadata["query_profile"]["evidence_need"] == ("vehicle_profile",)
    assert "license" in plate_metadata["query_profile"]["relation_variant_terms"]
    assert "plate" in plate_metadata["query_profile"]["relation_variant_terms"]

    assert "vehicle_profile" not in travel_metadata["query_profile"][
        "relation_categories"
    ]
    assert "vehicle_profile" not in dinner_plate_metadata["query_profile"][
        "relation_categories"
    ]
...
def test_query_decomposition_expands_age_profile_queries() -> None:
    ...
    assert how_old_queries[2] == "alex age old year birthday born turn"
    ...
    assert age_queries[2] == "alex age old year birthday born turn"
...
def test_query_decomposition_expands_activity_profile_queries() -> None:
    ...
    free_time_case = _case(
        case_id="activity-profile-free-time",
        question="What does Alex do in his free time?",
        expected_terms=("painting",),
        answer="painting",
    )
    ...
    free_time_queries, free_time_metadata = rerank_module.decomposed_search_queries(
        free_time_case
    )
    ...
    assert free_time_queries[2] == "alex activity hobby partake class creative fun express refresh"
    assert free_time_metadata["query_profile"]["relation_terms"] == (
        "activity",
        "hobby",
    )
    assert free_time_metadata["query_profile"]["relation_categories"] == (
        "activity",
        "activity_profile",
    )
    assert free_time_metadata["query_profile"]["evidence_need"] == (
        "activity_profile",
    )
    assert free_time_metadata["query_profile"]["time_intent_kind"] == "none"
    assert "temporal_support" not in free_time_metadata["query_plan"][
        "selected_roles"
    ]
...
def test_query_decomposition_expands_alias_profile_queries() -> None:
    ...
    go_by_case = _case(
        case_id="alias-profile-go-by",
        question="What name does Alex go by?",
        expected_terms=("Sunny",),
        answer="Sunny",
    )
    ...
    assert nickname_queries[2] == "alex nickname call name go by"
    ...
    assert call_name_queries[2] == "alex maria call nickname name go by"
    ...
    assert go_by_queries[2] == "alex nickname call name go by"
    assert go_by_metadata["query_profile"]["relation_terms"] == ("nickname",)
    assert go_by_metadata["query_profile"]["relation_categories"] == (
        "alias_profile",
    )
...
def test_query_decomposition_expands_date_profile_queries() -> None:
    ...
    wedding_date_case = _case(
        case_id="date-profile-wedding-date",
        question="What is Alex's wedding date?",
        expected_terms=("July 2",),
        answer="July 2",
        category=2,
    )
    birthday_day_case = _case(
        case_id="date-profile-birthday-day",
        question="What day is Alex's birthday?",
        expected_terms=("May 5",),
        answer="May 5",
    )
    birthday_month_case = _case(
        case_id="date-profile-birthday-month",
        question="What month is Alex's birthday?",
        expected_terms=("May",),
        answer="May",
        category=2,
    )
    date_of_birth_case = _case(
        case_id="date-profile-date-of-birth",
        question="What is Alex's date of birth?",
        expected_terms=("May 5",),
        answer="May 5",
        category=2,
    )
    ...
    anniversary_day_case = _case(
        case_id="date-profile-anniversary-day",
        question="Which day is Alex's anniversary?",
        expected_terms=("July 2",),
        answer="July 2",
    )
    ...
    wedding_venue_case = _case(
        case_id="date-profile-wedding-venue-guard",
        question="What wedding venue did Alex book?",
        expected_terms=("barn",),
        answer="barn",
    )
    birthday_duration_case = _case(
    ...
    wedding_date_queries, wedding_date_metadata = (
        rerank_module.decomposed_search_queries(wedding_date_case)
    )
    birthday_day_queries, birthday_day_metadata = (
        rerank_module.decomposed_search_queries(birthday_day_case)
    )
    birthday_month_queries, birthday_month_metadata = (
        rerank_module.decomposed_search_queries(birthday_month_case)
    )
    ...
    anniversary_day_queries, anniversary_day_metadata = (
        rerank_module.decomposed_search_queries(anniversary_day_case)
    )
    _, birthday_gift_metadata = rerank_module.decomposed_search_queries(
        birthday_gift_case
    )
    _, wedding_venue_metadata = rerank_module.decomposed_search_queries(
        wedding_venue_case
    )
    ...
    assert wedding_date_queries[1] == "alex anniversary year wed date month marry"
    assert wedding_date_metadata["query_profile"]["relation_terms"] == (
        "anniversary",
    )
    assert wedding_date_metadata["query_profile"]["relation_categories"] == (
        "date_profile",
        "temporal",
    )
    assert wedding_date_metadata["query_profile"]["evidence_need"] == (
        "temporal_support",
        "date_profile",
    )
    assert "date_support" in wedding_date_metadata["query_plan"]["selected_roles"]

    assert birthday_day_queries[2] == "alex birthday year ago born age"
    assert birthday_day_metadata["query_profile"]["relation_categories"] == (
        "date_profile",
    )
    assert birthday_day_metadata["query_profile"]["evidence_need"] == ("date_profile",)
    assert "date_support" in birthday_day_metadata["query_plan"]["selected_roles"]
    assert birthday_month_queries[1] == "alex birthday year ago born month age"
    assert "date_profile" in birthday_month_metadata["query_profile"][
        "relation_categories"
    ]
    assert "date_support" in birthday_month_metadata["query_plan"]["selected_roles"]
    assert date_of_birth_queries[1] == "alex birthday year ago born age"
    assert "dob" not in date_of_birth_metadata["query_profile"]["entities"]
    assert "date_profile" in date_of_birth_metadata["query_profile"][
        "relation_categories"
    ]
    ...
    assert dob_queries[2] == "alex birthday year ago born age"
    assert dob_metadata["query_profile"]["entities"] == ("alex",)
    assert dob_metadata["query_profile"]["relation_categories"] == ("date_profile",)
    assert dob_metadata["query_profile"]["evidence_need"] == ("date_profile",)
    assert anniversary_day_queries[2] == "alex anniversary year wed date month marry"
    assert anniversary_day_metadata["query_profile"]["relation_categories"] == (
        "date_profile",
    )
    ...
    assert "date_profile" not in wedding_venue_metadata["query_profile"][
        "relation_categories"
    ]
...
def test_query_decomposition_reports_communication_relation_intent() -> None:
    ...
    assert ask_queries[1].endswith("actions: ask, request, send, invoice, asked")
...
def test_benchmark_rerank_boosts_based_location_profile_evidence() -> None:
    case = _case(
        case_id="location-based-profile-rerank",
        question="Where is Alex based?",
        expected_terms=("Denver",),
        answer="Denver",
        category=4,
    ...
    based_profile = RetrievedMemory(
        item_id="based-profile",
        rank=2,
        score=0.0,
        text=(
            "session_2 turn D2:3 date: 10:15 am "
            "D2:3 Alex: I am based in Denver now."
        ),
        source_refs=("D2:3",),
    )
    ...
    reranked, metadata = rerank_module.benchmark_rerank_memories(
        case,
        (topical_location, based_profile),
    )

    assert metadata["applied"] is True
    assert metadata["query_profile"]["evidence_need"] == ("location_support",)
    assert reranked[0].item_id == "based-profile"
    diagnostics_by_id = {
        memory.item_id: memory.metadata["diagnostics"] for memory in reranked
    }
    based_diagnostics = diagnostics_by_id["based-profile"]
    topical_diagnostics = diagnostics_by_id["topical-location"]
    assert based_diagnostics["benchmark_candidate_features"][
        "relation_category_hits"
    ] == ["location_transition"]
    assert topical_diagnostics["benchmark_candidate_features"][
        "relation_category_hits"
    ...
    assert (
        based_diagnostics["score_signals"]["benchmark_location_support_boost"] > 0
    )


def test_benchmark_rerank_boosts_current_city_location_evidence() -> None:
    case = _case(
        case_id="location-current-city-rerank",
        question="What is Alex's current city?",
        expected_terms=("Denver",),
        answer="Denver",
        category=4,
    ...
    city_profile = RetrievedMemory(
        item_id="city-profile",
        rank=2,
        score=0.0,
        text=(
            "session_2 turn D2:3 date: 10:15 am "
            "D2:3 Alex: My current city is Denver."
        ),
        source_refs=("D2:3",),
    )
    ...
    reranked, metadata = rerank_module.benchmark_rerank_memories(
        case,
        (topical_location, city_profile),
    )

    assert metadata["applied"] is True
    assert metadata["query_profile"]["evidence_need"] == ("location_support",)
    assert reranked[0].item_id == "city-profile"
    diagnostics_by_id = {
        memory.item_id: memory.metadata["diagnostics"] for memory in reranked
    }
    city_diagnostics = diagnostics_by_id["city-profile"]
    topical_diagnostics = diagnostics_by_id["topical-location"]
    assert city_diagnostics["benchmark_candidate_features"][
        "relation_category_hits"
    ] == ["location_transition"]
    assert topical_diagnostics["benchmark_candidate_features"][
        "relation_category_hits"
    ] == []
    assert city_diagnostics["score_signals"]["benchmark_location_support_boost"] > 0
    assert (
        topical_diagnostics["score_signals"]["benchmark_location_support_boost"]
        == 0
    )
...
def test_benchmark_rerank_boosts_staying_location_evidence() -> None:
    case = _case(
        case_id="location-staying-rerank",
        question="Where is Alex staying?",
        expected_terms=("Hilton",),
        answer="Hilton",
        category=4,
    ...
    topical_location = RetrievedMemory(
        ...
        text=(
            "session_1 turn D1:1 date: 10:00 am "
            "D1:1 Alex mentioned the Hilton during the planning chat."
        ),
        source_refs=("D1:1",),
    )
    stay_profile = RetrievedMemory(
        item_id="stay-profile",
        rank=2,
        score=0.0,
        text=(
            "session_2 turn D2:3 date: 10:15 am "
            "D2:3 Alex: I am staying at the Hilton."
        ),
        source_refs=("D2:3",),
    )
    ...
    reranked, metadata = rerank_module.benchmark_rerank_memories(
        case,
        (topical_location, stay_profile),
    )

    assert metadata["applied"] is True
    assert metadata["query_profile"]["evidence_need"] == ("location_support",)
    assert reranked[0].item_id == "stay-profile"
    diagnostics_by_id = {
        memory.item_id: memory.metadata["diagnostics"] for memory in reranked
    }
    stay_diagnostics = diagnostics_by_id["stay-profile"]
    topical_diagnostics = diagnostics_by_id["topical-location"]
    assert stay_diagnostics["benchmark_candidate_features"][
        "relation_category_hits"
    ] == ["location_transition"]
    assert topical_diagnostics["benchmark_candidate_features"][
        "relation_category_hits"
    ] == []
    assert stay_diagnostics["score_signals"]["benchmark_location_support_boost"] > 0
    assert (
        topical_diagnostics["score_signals"]["benchmark_location_support_boost"]
        == 0
    )
...
def test_benchmark_rerank_boosts_raised_origin_profile_evidence() -> None:
    case = _case(
        case_id="location-raised-origin-rerank",
        question="Where was Alex raised?",
        expected_terms=("Toronto",),
        answer="Toronto",
        category=4,
    ...
    topical_origin = RetrievedMemory(
        ...
        text=(
            "session_1 turn D1:1 date: 10:00 am "
            "D1:1 Alex joined a fundraiser raising awareness in Toronto."
        ),
        source_refs=("D1:1",),
    )
    raised_profile = RetrievedMemory(
        item_id="raised-profile",
        rank=2,
        score=0.0,
        text=(
            "session_2 turn D2:3 date: 10:15 am "
            "D2:3 Alex: I was raised in Toronto."
        ),
        source_refs=("D2:3",),
    )
    ...
    reranked, metadata = rerank_module.benchmark_rerank_memories(
        case,
        (topical_origin, raised_profile),
    )

    assert metadata["applied"] is True
    assert metadata["query_profile"]["evidence_need"] == ("location_support",)
    assert reranked[0].item_id == "raised-profile"
    diagnostics_by_id = {
        memory.item_id: memory.metadata["diagnostics"] for memory in reranked
    }
    raised_diagnostics = diagnostics_by_id["raised-profile"]
    topical_diagnostics = diagnostics_by_id["topical-origin"]
    assert raised_diagnostics["benchmark_candidate_features"][
        "relation_category_hits"
    ] == ["location_transition"]
    assert topical_diagnostics["benchmark_candidate_features"][
        "relation_category_hits"
    ...
    assert (
        raised_diagnostics["score_signals"]["benchmark_location_support_boost"] > 0
    )
    assert (
        topical_diagnostics["score_signals"]["benchmark_location_support_boost"]
        == 0
...
def test_benchmark_rerank_boosts_primary_care_physician_evidence() -> None:
    case = _case(
        case_id="health-profile-physician-rerank",
        question="Who is Alex's primary care physician?",
        expected_terms=("Dr. Lee",),
        answer="Dr. Lee",
        category=4,
    )
    topical_physician = RetrievedMemory(
        item_id="topical-physician",
        rank=1,
        score=0.2,
        text=(
            "session_1 turn D1:1 date: 10:00 am "
            "D1:1 Alex read an article about physicians with Maria."
        ),
        source_refs=("D1:1",),
    )
    health_profile = RetrievedMemory(
        ...
        text=(
            "session_2 turn D2:3 date: 10:15 am "
            "D2:3 Alex: My primary care physician is Dr. Lee."
        ),
        source_refs=("D2:3",),
    )
    ...
    reranked, metadata = rerank_module.benchmark_rerank_memories(
        case,
        (topical_physician, health_profile),
    )
...
def test_query_decomposition_expands_diet_avoidance_queries() -> None:
    avoid_case = _case(
        ...
        question="What food does Alex avoid?",
    ...
    assert avoid_metadata["query_profile"]["relation_terms"] == ("diet",)
    assert avoid_metadata["query_profile"]["relation_categories"] == (
        "diet_profile",
    )
...
def test_benchmark_rerank_boosts_diet_avoidance_evidence() -> None:
    case = _case(
        case_id="diet-profile-avoidance-rerank",
        question="What food does Alex avoid?",
        expected_terms=("seafood",),
        answer="seafood",
        category=4,
    ...
    topical_diet = RetrievedMemory(
        ...
        text=(
            "session_1 turn D1:1 date: 10:00 am "
            "D1:1 Alex reviewed seafood restaurants with Maria."
        ),
        source_refs=("D1:1",),
    )
    diet_profile = RetrievedMemory(
...
def test_benchmark_rerank_boosts_license_plate_vehicle_evidence() -> None:
    case = _case(
        case_id="vehicle-license-plate-rerank",
        question="What is Alex's license plate?",
        expected_terms=("ABC-123",),
        answer="ABC-123",
        category=4,
    )
    topical_plate = RetrievedMemory(
        item_id="topical-plate",
        rank=1,
        score=0.2,
        text=(
            "session_1 turn D1:1 date: 10:00 am "
            "D1:1 Alex used the blue plate for dinner with Maria."
        ),
        source_refs=("D1:1",),
    )
    vehicle_profile = RetrievedMemory(
        ...
        text=(
            "session_2 turn D2:3 date: 10:15 am "
            "D2:3 Alex: My license plate is ABC-123."
        ),
        source_refs=("D2:3",),
    )
    ...
    vehicle_diagnostics = diagnostics_by_id["vehicle-profile"]
    topical_diagnostics = diagnostics_by_id["topical-plate"]
    assert vehicle_diagnostics["benchmark_candidate_features"][
        "relation_category_hits"
    ] == ["vehicle_profile"]
    assert topical_diagnostics["benchmark_candidate_features"][
...
def test_benchmark_rerank_boosts_turning_age_profile_evidence() -> None:
    case = _case(
        case_id="age-profile-turning-rerank",
        question="How old is Alex?",
        expected_terms=("32",),
        answer="32",
        category=4,
    )
    topical_turning = RetrievedMemory(
        item_id="topical-turning",
        rank=1,
        score=0.2,
        text=(
            "session_1 turn D1:1 date: 10:00 am "
            "D1:1 Alex talked about turning pages in a 32-page book."
        ),
        source_refs=("D1:1",),
    )
    age_profile = RetrievedMemory(
        ...
        text=(
            "session_2 turn D2:3 date: 10:15 am "
            "D2:3 Alex: I'm turning 32 next month."
        ),
        source_refs=("D2:3",),
    )
    ...
    age_diagnostics = diagnostics_by_id["age-profile"]
    topical_diagnostics = diagnostics_by_id["topical-turning"]
    assert age_diagnostics["benchmark_candidate_features"][
        "relation_category_hits"
    ] == ["age_profile"]
    assert topical_diagnostics["benchmark_candidate_features"][
...
def test_benchmark_rerank_boosts_go_by_alias_profile_evidence() -> None:
    case = _case(
        case_id="alias-profile-go-by-rerank",
        question="What name does Alex go by?",
        expected_terms=("Sunny",),
        answer="Sunny",
        category=4,
    )
    topical_go_by = RetrievedMemory(
        item_id="topical-go-by",
        rank=1,
        score=0.2,
        text=(
            "session_1 turn D1:1 date: 10:00 am "
            "D1:1 Alex went by the pharmacy on the way home."
        ),
        source_refs=("D1:1",),
    )
    alias_profile = RetrievedMemory(
    ...
    alias_diagnostics = diagnostics_by_id["alias-profile"]
    topical_diagnostics = diagnostics_by_id["topical-go-by"]
    assert alias_diagnostics["benchmark_candidate_features"][
        "relation_category_hits"
    ] == ["alias_profile"]
    assert topical_diagnostics["benchmark_candidate_features"][
...
def test_benchmark_rerank_boosts_free_time_activity_evidence() -> None:
    case = _case(
        case_id="activity-profile-free-time-rerank",
        question="What does Alex do in his free time?",
        expected_terms=("painting",),
        answer="painting",
        category=4,
    )
    topical_activity = RetrievedMemory(
        item_id="topical-activity",
        rank=1,
        score=0.2,
        text=(
            "session_1 turn D1:1 date: 10:00 am "
            "D1:1 Alex read an article about free time management."
        ),
        source_refs=("D1:1",),
    )
    activity_profile = RetrievedMemory(
        ...
        text=(
            "session_2 turn D2:3 date: 10:15 am "
            "D2:3 Alex: In my free time, I enjoy painting."
        ),
        source_refs=("D2:3",),
    )
    ...
    reranked, metadata = rerank_module.benchmark_rerank_memories(
        case,
        (topical_activity, activity_profile),
    )

    assert metadata["applied"] is True
    assert metadata["query_profile"]["evidence_need"] == ("activity_profile",)
    assert metadata["query_profile"]["time_intent_kind"] == "none"
    assert reranked[0].item_id == "activity-profile"
    diagnostics_by_id = {
        memory.item_id: memory.metadata["diagnostics"] for memory in reranked
    }
    activity_diagnostics = diagnostics_by_id["activity-profile"]
    topical_diagnostics = diagnostics_by_id["topical-activity"]
    assert "activity_profile" in activity_diagnostics["benchmark_candidate_features"][
        "relation_category_hits"
    ]
    assert "activity_profile" not in topical_diagnostics[
...
def test_benchmark_rerank_boosts_birthday_day_date_profile_evidence() -> None:
    case = _case(
        case_id="date-profile-birthday-day-rerank",
        question="What day is Alex's birthday?",
        expected_terms=("May 5",),
        answer="May 5",
        category=4,
...
def test_benchmark_rerank_boosts_dob_date_profile_evidence() -> None:
    case = _case(
        case_id="date-profile-dob-rerank",
        question="What is Alex's DOB?",
        expected_terms=("May 5",),
        answer="May 5",
        category=4,
...
def test_benchmark_rerank_boosts_wedding_date_evidence() -> None:
    case = _case(
        case_id="date-profile-wedding-date-rerank",
        question="What is Alex's wedding date?",
        expected_terms=("July 2",),
        answer="July 2",
        category=4,
    ...
    topical_wedding = RetrievedMemory(
        ...
        text=(
            "session_1 turn D1:1 date: 10:00 am "
            "D1:1 Alex attended a wedding venue tour in July."
        ),
        source_refs=("D1:1",),
    )
    date_profile = RetrievedMemory(
        ...
        text=(
            "session_2 turn D2:3 date: 10:15 am "
            "D2:3 Alex: Our wedding date is July 2."
        ),
        source_refs=("D2:3",),
    )
    ...
    assert "date_profile" not in topical_diagnostics["benchmark_candidate_features"][
        "relation_category_hits"
    ]
    assert date_diagnostics["score_signals"][
        "benchmark_typed_relation_support_roles"
