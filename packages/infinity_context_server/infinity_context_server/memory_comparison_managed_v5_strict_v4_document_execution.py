"""Recovered strict-v4 document authority for the production composition root."""
...
from infinity_context_core.ports.managed_cleanup_v3_contracts import LONGMEMEVAL_PROFILE
...
@final
class RecoveredStrictV4DocumentAuthority:
    """Own the authenticated index snapshot used by document admission."""

    def __init__(
        self,
        index: SQLiteManagedCleanupV3ExpectedRowAuthority,
        corpus_delegate: ExpectedIndexStrictV4FactAuthority,
        document_delegate: ExpectedIndexStrictV4DocumentAuthority,
        receipt: StrictV4PreparationReceipt,
    ) -> None:
        self._index = index
        self._corpus_delegate = corpus_delegate
        self._document_delegate = document_delegate
    ...
    def admit_corpus(
        ...
        self._require_open()
        return self._corpus_delegate.admit_corpus(claim)

    def admit_document(
        self, claim: ManagedBenchmarkStrictV4DocumentClaim
    ) -> ManagedBenchmarkStrictV4DocumentAdmission:
        self._require_open()
        return self._document_delegate.admit_document(claim)
    ...
    def __enter__(self) -> RecoveredStrictV4DocumentAuthority:
        self._require_open()
        return self
    ...
    def _require_open(self) -> None:
        if self._closed:
            raise ProjectionReceiptError("projection_receipt.document_authority_closed")


async def recover_strict_v4_document_authority(
    *,
    ...
    expected_projector: ManagedV5CleanupV4OperationProjector,
) -> RecoveredStrictV4DocumentAuthority:
    """Reauthenticate all artifacts, then pin the exact document authority."""
    ...
    try:
        corpus = ExpectedIndexStrictV4FactAuthority(
            context=receipt.a2_context,
            authority_terminal_sha256=receipt.expected_index_terminal_sha256,
            lookup=index,
        )
        document = ExpectedIndexStrictV4DocumentAuthority(
            context=receipt.a2_context,
            authority_terminal_sha256=receipt.expected_index_terminal_sha256,
            lookup=index,
        )
        return RecoveredStrictV4DocumentAuthority(index, corpus, document, receipt)
...
def _validate_execution_projector(
    ...
    if (
        ...
        raise ProjectionReceiptError("projection_receipt.document_projection_invalid")
    ...
    if (
        receipt.profile_id != LONGMEMEVAL_PROFILE
        ...
        or context.case_manifest_sha256 != projector.projection.case_manifest_sha256
        or context.publishable_profile_commitment_sha256
        ...
        or pair is None
        or receipt.original_pair_terminal_sha256 != pair.terminal_commitment_sha256
    ):
        raise ProjectionReceiptError("projection_receipt.document_projection_invalid")
