...
def test_answer_context_fallback_filters_private_provider_source_labels() -> None:
    context = answer_context_from_evidence_bundle(
        (
            RetrievedMemory(
                text="D1:1 Morgan chose the evening class.",
    ...
    prompt = render_answer_prompt(
        PublicBenchmarkCase(
            benchmark="locomo",
            case_id="conv-1:qa:fallback-source-label-prompt",
            question="Which class did Morgan choose?",
            expected_terms=("evening class",),
            memory_scope_external_ref="locomo-conv-1",
            thread_external_ref="locomo-conv-1",
...
def test_answer_context_uses_bundle_order_within_cutoff() -> None:
    ...
    assert context.to_diagnostics() == {
        ...
        "inspection_flags": ["missing_required_roles"],
...
def test_answer_context_metrics_aggregates_sources_and_compression() -> None:
                ...
                "cutoff_results": {
                    "3": {
                        ...
                        "answer_context": {
                            ...
                            "compacted_fusion_source_ref_original_count": 4,
                            "compacted_fusion_source_ref_selected_count": 1,
    ...
    assert metrics["primary_total_compacted_fusion_source_ref_original_count"] == 4
    assert metrics["primary_total_compacted_fusion_source_ref_selected_count"] == 1
    ...
    assert primary["avg_compacted_fusion_source_ref_original_count"] == 2.0
    assert primary["total_compacted_fusion_source_ref_original_count"] == 4
    assert primary["avg_compacted_fusion_source_ref_selected_count"] == 0.5
    assert primary["total_compacted_fusion_source_ref_selected_count"] == 1
