"""Deterministic weighted-RRF orchestration for locator-only Retrieval."""
...
from infinity_context_core.features.context_building.domain.locator_retrieval import (
    ...
    LOCATOR_RETRIEVAL_RANK_CONSTANT,
    LOCATOR_RETRIEVAL_SCORE_SCALE_PICOS,
    MAX_PROVIDER_REGISTRATIONS,
...
def _rrf_contribution_score_picos(
    ...
    denominator = total_query_weight_micros * (LOCATOR_RETRIEVAL_RANK_CONSTANT + provider_rank)
...
class _ProviderExecution:
    registration: LocatorProviderRegistration
    result: LocatorProviderResult
...
@dataclass(frozen=True, slots=True)
class RetrieveLocators:
    """Execute generic fan-in, canonical filtering, fusion and neighbor attachment."""

    providers: tuple[LocatorProviderRegistration, ...]
    canonical_reader: CanonicalLocatorReadPort
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.capability, LocatorRetrievalCapability):
            raise ValueError("Locator retrieval requires a trusted capability descriptor")

    async def execute(self, request: LocatorRetrievalRequest) -> LocatorRetrievalResponse:
        ...
        response = LocatorRetrievalResponse(
    ...
    async def _execute_provider(
        ...
        try:
            ...
            result = LocatorProviderResult(status="unavailable", reason_code="provider_error")
        return _ProviderExecution(registration, result)

    async def _hydrate_final_results(
        self,
        request: LocatorRetrievalRequest,
        selected: tuple[LocatorResultCandidate, ...],
    ) -> tuple[LocatorResultCandidate, ...]:
        if not selected:
            return ()
        ...
        seeds = {
            ...
            if candidate_matches_request(item, request)
        }
        ...
        final: list[LocatorResultCandidate] = []
        proposed: dict[str, tuple[LocatorNeighbor, ...]] = {}
...
def _registration_tuple(values: object) -> tuple[LocatorProviderRegistration, ...]:
    ...
    registrations = tuple(
        ...
        if isinstance(item, LocatorProviderRegistration)
        else _raise_invalid_registration()
        for item in source
    )
    if not 1 <= len(registrations) <= MAX_PROVIDER_REGISTRATIONS:
...
def _validated_request_copy(value: object) -> LocatorRetrievalRequest:
    ...
    queries = tuple(
        ...
        if isinstance(item, LocatorQueryVariant)
    ...
    hard_interval = (
        None
        if interval is None
        else LocatorTimeInterval(interval.start_at, interval.end_at)
        if isinstance(interval, LocatorTimeInterval)
    ...
    hard_relative = (
        None
        if relative is None
        else LocatorRelativeTimeInterval(relative.start_ms, relative.end_ms)
        if isinstance(relative, LocatorRelativeTimeInterval)
    ...
    soft_interval = (
        ...
        else LocatorTimeInterval(preference_interval.start_at, preference_interval.end_at)
        if isinstance(preference_interval, LocatorTimeInterval)
        else _raise_invalid_interval()
    )
    ...
    soft_relative = (
        ...
        else LocatorRelativeTimeInterval(preference_relative.start_ms, preference_relative.end_ms)
        if isinstance(preference_relative, LocatorRelativeTimeInterval)
        else _raise_invalid_relative_interval()
    )
    return LocatorRetrievalRequest(
        LOCATOR_RETRIEVAL_CONTRACT_VERSION
        if value.contract_version == LOCATOR_RETRIEVAL_CONTRACT_VERSION
        ...
        LocatorHardFilters(
            tuple(
                LocatorSourceGeneration(item.source_key, item.projection_generation)
                if isinstance(item, LocatorSourceGeneration)
        ...
        LocatorSoftPreferences(
            tuple(
                LocatorWeightedKey(item.key, item.weight_micros)
                if isinstance(item, LocatorWeightedKey)
...
def _raise_invalid_query() -> LocatorQueryVariant:
    raise ValueError("Locator queries contain an invalid runtime type")


def _raise_invalid_interval() -> LocatorTimeInterval:
    raise ValueError("Locator interval has an invalid runtime type")


def _raise_invalid_relative_interval() -> LocatorRelativeTimeInterval:
    raise ValueError("Locator relative interval has an invalid runtime type")


def _raise_invalid_source_generation() -> LocatorSourceGeneration:
    raise ValueError("Locator source_generations contains an invalid runtime type")


def _raise_invalid_weighted_key() -> LocatorWeightedKey:
    raise ValueError("Locator preferences contain an invalid runtime type")


def _validated_provider_result_copy(value: object) -> LocatorProviderResult:
    if not isinstance(value, LocatorProviderResult):
    ...
    hits = tuple(
        ...
        if isinstance(item, LocatorProviderHit)
        else _raise_invalid_hit()
        for item in tuple(value.hits)
    )
    return LocatorProviderResult(hits, value.status, value.reason_code)


def _raise_invalid_hit() -> LocatorProviderHit:
    raise ValueError("Locator provider hits contain an invalid runtime type")


def _validated_canonical_copy(value: object) -> CanonicalLocatorCandidate:
    if not isinstance(value, CanonicalLocatorCandidate):
...
def _validated_read_copy(value: object) -> CanonicalLocatorRead:
    if not isinstance(value, CanonicalLocatorRead):
        raise CanonicalHydrationInvariantError(
    ...
    try:
        ...
        return CanonicalLocatorRead(seeds, neighbors)
    except CanonicalHydrationInvariantError:
        raise
    except (TypeError, ValueError) as error:
        raise CanonicalHydrationInvariantError(
...
def _canonical_mapping(
    request: LocatorRetrievalRequest, values: object, label: str
) -> dict[str, CanonicalLocatorCandidate]:
    if isinstance(values, str | bytes):
        raise CanonicalHydrationInvariantError(f"{label} returned a malformed collection")
    try:
...
def _assert_snapshot_and_identity_invariants(
    candidates: tuple[CanonicalLocatorCandidate, ...], label: str
) -> None:
    snapshots = {item.read_snapshot for item in candidates}
    if len(snapshots) > 1:
        raise CanonicalHydrationInvariantError(f"{label} mixed read snapshots")
    identities = tuple(item.canonical_identity for item in candidates)
    if len(set(identities)) != len(identities):
        raise CanonicalHydrationInvariantError(f"{label} returned duplicate identities")
...
def _fuse(
    ...
    unique: dict[tuple[str, str, str], tuple[LocatorProviderHit, int]] = {}
    ...
    grouped: dict[tuple[str, int], list[LocatorScoreContribution]] = {}
...
def _hit_order(hit: LocatorProviderHit) -> tuple[int, int, str, float]:
    return (
        hit.provider_rank,
...
def _select_results(
    request: LocatorRetrievalRequest,
    fused: tuple[_Fused, ...],
    canonical: dict[str, CanonicalLocatorCandidate],
) -> tuple[LocatorResultCandidate, ...]:
    selected: list[LocatorResultCandidate] = []
    for item in fused:
        ...
        selected.append(
            LocatorResultCandidate(
                ...
                item.base_score_picos / LOCATOR_RETRIEVAL_SCORE_SCALE_PICOS,
                tuple(
                    sorted(
...
def _same_seed_coordinates(
    ranked: LocatorResultCandidate, hydrated: CanonicalLocatorCandidate
) -> bool:
    return (
...
def _neighbors_for_seed(
    seed: CanonicalLocatorCandidate,
    candidates: tuple[CanonicalLocatorCandidate, ...],
    radius: int,
) -> tuple[LocatorNeighbor, ...]:
    if seed.sequence_ordinal is None or radius == 0:
        return ()
    positions: dict[int, CanonicalLocatorCandidate] = {}
...
def _provider_is_qualified(result: LocatorProviderResult) -> bool:
...
def _response_size_bytes(response: LocatorRetrievalResponse) -> int:
    encoded = json.dumps(
...
def _bounded_response(
    ...
    capability: LocatorRetrievalCapability,
    response: LocatorRetrievalResponse,
    outcomes: tuple[LocatorProviderOutcome, ...],
...
__all__ = ("LocatorProviderRegistration", "RetrieveLocators")
