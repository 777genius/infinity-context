"""Strict JSON parser for the canonical Retrieval V2 response envelope."""

from __future__ import annotations

from collections.abc import Mapping, Sequence

from ._context_building_retrieval_v2 import (
...
def parse_retrieve_context_v2_response(
...
def _candidate(payload: Mapping[str, object], index: int) -> RetrievalV2CandidateDto:
...
def _outcome(payload: Mapping[str, object], index: int) -> RetrievalV2ProviderOutcomeDto:
    path = f"provider_outcomes.{index}"
    _keys(payload, {"provider_id", "status", "reason_code"}, path)
    reason = payload.get("reason_code")
    return RetrievalV2ProviderOutcomeDto(
        provider_id=_string(payload, "provider_id", path),
...
__all__ = ("parse_retrieve_context_v2_response",)
