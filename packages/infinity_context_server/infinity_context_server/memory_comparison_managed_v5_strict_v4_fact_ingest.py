"""Provider-free strict-v4 LoCoMo ingestion through production fact handlers."""
...
STRICT_V4_FACT_INGEST_SCHEMA = "memory-comparison-strict-v4-fact-ingest.v1"
_ROOT_DOMAIN = b"memory-comparison-strict-v4/fact-ingest-root/v1\0"
_SHA256 = re.compile(r"^[0-9a-f]{64}$")


class StrictV4FactIngestError(RuntimeError):
    """Stable provider-free execution failure."""


@final
@dataclass(frozen=True, slots=True)
class StrictV4FactIngestReceipt:
    profile_id: str
    run_id_sha256: str
    context_sha256: str
    ...
    schema_version: str = STRICT_V4_FACT_INGEST_SCHEMA

    def __post_init__(self) -> None:
        if (
            self.schema_version != STRICT_V4_FACT_INGEST_SCHEMA
            or self.profile_id != LOCOMO_PROFILE
            or any(
            ...
            or self.replayed_count > self.operation_count
            or type(self.provider_calls) is not int
            or self.provider_calls != 0
        ):
            raise StrictV4FactIngestError("strict_v4_fact_ingest_receipt_invalid")
...
@final
class StrictV4FactIngestRuntime:
    """Own the exact DB composition used by one recoverable ingestion pass."""

    def __init__(
        self,
        *,
        database_url: str,
        authority: ManagedBenchmarkStrictV4FactAuthorityPort,
    ) -> None:
        if type(database_url) is not str or not database_url:
            raise StrictV4FactIngestError("strict_v4_fact_ingest_database_invalid")
    ...
    async def execute(
        ...
        *,
    ...
    ) -> StrictV4FactIngestReceipt:
        if self._closed:
            raise StrictV4FactIngestError("strict_v4_fact_ingest_runtime_closed")
        if (
            type(projector) is not ManagedV5CleanupV4OperationProjector
            or projector.profile_id != LOCOMO_PROFILE
            or type(space_slug) is not str
            or not space_slug
        ):
            raise StrictV4FactIngestError("strict_v4_fact_ingest_input_invalid")
        authenticate_strict_v4_preparation_receipt(
            preparation_receipt,
        ...
        if (
            ...
            raise StrictV4FactIngestError("strict_v4_fact_ingest_binding_invalid")
        root = hashlib.sha256(_ROOT_DOMAIN).digest()
        corpus_count = operation_count = replayed_count = 0
        for corpus in projector.iter_reconstructed_corpora():
            ...
            if type(scope_ref) is not str or type(thread_ref) is not str:
                raise StrictV4FactIngestError("strict_v4_fact_ingest_corpus_invalid")
            ...
            for memory in corpus.memories:
                source_id = memory.source_external_id
                if type(source_id) is not str or not source_id:
                    raise StrictV4FactIngestError("strict_v4_fact_ingest_source_invalid")
                result = await self._lifecycle.remember_fact.execute(
                    RememberFactCommand(
                        scope=MemoryFactScope(
                            str(scope.space_id),
                            str(scope.memory_scope_id),
                            str(scope.thread_id) if scope.thread_id is not None else None,
                        ),
                        text=memory.text,
                        source_refs=(_fact_source_ref(source_id, memory.text),),
                        kind=memory.kind,
                    )
                )
                operation_count += 1
                replayed_count += int(result.replayed)
                root = _extend_root(
                    root,
                    operation_count - 1,
                    source_id,
                    result.fact.identity.fact_id,
                    result.fact.visibility.version,
                    result.outbox_message_ids,
                )
        expected = projector.manifest_authority.operation_count
        if operation_count != expected or corpus_count != projector.manifest_authority.corpus_count:
            raise StrictV4FactIngestError("strict_v4_fact_ingest_count_invalid")
        return build_strict_v4_fact_ingest_receipt(
...
def _fact_source_ref(source_id: str, text: str) -> MemoryFactSourceRef:
    """Build the exact evidence preview committed by the sealed projector."""

    return MemoryFactSourceRef(
        source_type="memory_comparison_benchmark",
        source_id=source_id,
        quote_preview=safe_preview(text),
    )
    ...
    payload = json.dumps(
        {
            ...
            "fact_id": fact_id,
            "version": version,
            "outbox_message_ids": list(outbox_message_ids),
        },
        sort_keys=True,
        separators=(",", ":"),
        ensure_ascii=True,
...
def build_strict_v4_fact_ingest_receipt(
    *,
    preparation_receipt: StrictV4PreparationReceipt,
    authenticator: ProjectionReceiptAuthenticator,
    corpus_count: int,
    ...
    base = StrictV4FactIngestReceipt(
    ...
    receipt_sha256 = commitment(
        "strict-v4-fact-ingest/v1",
        base.payload(authenticated=False),
    )
    return StrictV4FactIngestReceipt(
        **{
        ...
        receipt_mac_sha256=authenticator.sign(
            "strict-v4-fact-ingest",
            receipt_sha256,
        ),
    )


def authenticate_strict_v4_fact_ingest_receipt(
    receipt: StrictV4FactIngestReceipt,
    *,
    authenticator: ProjectionReceiptAuthenticator,
) -> None:
    if type(receipt) is not StrictV4FactIngestReceipt:
        raise StrictV4FactIngestError("strict_v4_fact_ingest_receipt_invalid")
...
__all__ = (
    "STRICT_V4_FACT_INGEST_SCHEMA",
    "StrictV4FactIngestError",
    "StrictV4FactIngestReceipt",
    "StrictV4FactIngestRuntime",
    "authenticate_strict_v4_fact_ingest_receipt",
    "build_strict_v4_fact_ingest_receipt",
)
