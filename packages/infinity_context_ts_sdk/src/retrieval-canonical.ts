...
export function canonicalRetrievalCapabilityBytes(value: unknown): Uint8Array {
...
export async function retrievalCapabilityFingerprint(value: unknown): Promise<string> {
  const subtle = globalThis.crypto?.subtle;
  if (subtle === undefined) canonicalCapabilityInvalid("Web Crypto SHA-256 is unavailable");
  const canonicalBytes = canonicalRetrievalCapabilityBytes(value);
...
export async function verifyRetrievalCapabilityFingerprint(
  capability: RetrievalCapability,
): Promise<void> {
  const actual = await retrievalCapabilityFingerprint(capability);
