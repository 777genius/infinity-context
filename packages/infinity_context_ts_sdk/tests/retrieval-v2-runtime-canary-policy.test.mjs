...
import { assertRetrievalV2RuntimeCanary } from "../scripts/retrieval-v2-runtime-canary-policy.mjs";
...
describe("Retrieval V2 runtime canary policy", () => {
  it("accepts deterministic optional degradation without blocking a healthy required lane", () => {
    expect(() =>
      assertRetrievalV2RuntimeCanary({ capability, response, ...expected }),
    ).not.toThrow();
  });
  ...
  it.each([
    ...
    expect(() =>
      assertRetrievalV2RuntimeCanary({ capability, response: candidate, ...expected }),
    ).toThrow();
  });
  ...
  it("rejects fingerprint, profile and revision drift", () => {
    for (const key of [
      ...
      expect(() =>
        assertRetrievalV2RuntimeCanary({ capability, response, ...expected, [key]: "wrong" }),
